...
For BCL Convert run-analysis launches, DYEC patches the active DayOA profile for direct mounted input, zero barcode mismatches, `merge_lane_fastqs: false`, `merge_tile_fastqs: false`, and `shared_thread_odirect_output: auto`; the launcher then hard-requires a DayOA checkout that exposes native lane-split BCL rules and lane-level downstream report consumption.
