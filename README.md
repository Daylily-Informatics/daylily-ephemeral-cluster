...
catalog action. The `19.0.9` snapshot is immutable; there is no mutable
`current` alias or implicit catalog selection:

```bash
dyec --json catalog list --dyec-version 19.0.9 --type prod
dyec --json catalog render <command-id> --dyec-version 19.0.9 ...
```
...
The `19.0.9` catalog targets DayOA `16.0.4`. Public catalog output includes a
