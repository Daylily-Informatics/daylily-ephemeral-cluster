...
catalog action. The `19.0.17` snapshot is immutable; there is no mutable
`current` alias or implicit catalog selection:

```bash
dyec --json catalog list --dyec-version 19.0.17 --type prod
dyec --json catalog render <command-id> --dyec-version 19.0.17 ...
```
...
The `19.0.17` catalog targets DayOA `16.0.4`. Public catalog output includes a
...
dyec --json catalog validation-compare <command-id> \
  --dyec-version 19.0.17 --profile "$AWS_PROFILE" --region "$REGION"
```
...
Release tags are numeric, annotated semver tags with no leading `v`. Do not move pushed tags. The checked-in release target is `19.0.17`; tag only the exact clean release commit after required acceptance.
