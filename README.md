...
catalog action. The `19.0.25` snapshot is immutable; there is no mutable
`current` alias or implicit catalog selection:

```bash
dyec --json catalog list --dyec-version 19.0.25 --type prod
dyec --json catalog render <command-id> --dyec-version 19.0.25 ...
```
...
The `19.0.25` catalog preserves the `19.0.24` command shapes and targets DayOA
...
dyec --json catalog validation-compare <command-id> \
  --dyec-version 19.0.25 --profile "$AWS_PROFILE" --region "$REGION"
```
...
Release tags are numeric, annotated semver tags with no leading `v`. Do not move pushed tags. The checked-in release target is `19.0.25`; tag only the exact clean release commit after required acceptance.
