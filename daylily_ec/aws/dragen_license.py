"""Read-only validation for the private DRAGEN credential secret policy."""
...
def make_dragen_license_preflight_step(
    *,
    ...
    """Return a preflight step that validates metadata and least privilege only."""
    ...
    def step(report: PreflightReport) -> PreflightReport:
        try:
            ...
            if secret.get("DeletedDate") is not None:
                raise ValueError("Configured credential secret is scheduled for deletion.")
            if secret.get("KmsKeyId"):
                raise ValueError(
                    "Configured credential secret uses a customer KMS key; this contract "
            ...
            report.checks.append(
                CheckResult(
                    ...
                    remediation=(
                        "Create the configured secret and a managed policy granting only "
                        "secretsmanager:DescribeSecret and secretsmanager:GetSecretValue "
                        "on that exact secret ARN."
                    ),
                )
            )
            return report
