...
validate_aws_profile() {
  ...
  if [[ -z "${AWS_PROFILE:-}" ]]; then
    echo "ERROR: AWS_PROFILE environment variable is not set. Please set AWS_PROFILE before running this script." >&2
    exit 1
  fi
  ...
  if [[ "$PROFILE_FOUND" == false ]]; then
    echo "ERROR: AWS profile '$AWS_PROFILE' was not found in $HOME/.aws/credentials or $HOME/.aws/config." >&2
    echo "Please run 'aws configure --profile $AWS_PROFILE' to create the profile and try again." >&2
    exit 1
  fi
}
