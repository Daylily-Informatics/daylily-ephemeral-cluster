...
def _build_headnode_dayec_install_command(repo_name: str) -> str:
    ...
    return "\n".join(
        (
            ...
            'cd "$repo_dir"',
            'source "$HOME/miniconda3/etc/profile.d/conda.sh"',
            'dayec_conda_pkgs_root="$HOME/.cache/daylily/conda-pkgs"',
            'install -d -m 0700 "$dayec_conda_pkgs_root"',
            'dayec_conda_pkgs="$(mktemp -d "$dayec_conda_pkgs_root/configure.XXXXXX")"',
            'cleanup_dayec_conda_pkgs() {',
            '  case "$dayec_conda_pkgs" in',
            '    "$dayec_conda_pkgs_root"/configure.*) rm -rf -- "$dayec_conda_pkgs" ;;',
            '    *) echo "Refusing to remove unexpected Conda package cache: $dayec_conda_pkgs" >&2; return 1 ;;',
            '  esac',
            '}',
            'trap cleanup_dayec_conda_pkgs EXIT',
            'export CONDA_PKGS_DIRS="$dayec_conda_pkgs"',
