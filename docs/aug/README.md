...
12. **Workset Monitor & Queue Management** with REST API, concurrent processing, automatic retry, and customer portal for multi-tenant workload management.

---

## Workset Monitor & Queue Management

The Daylily Workset Monitor provides enterprise-grade workload orchestration and monitoring capabilities:

### Core Features

- **🚀 Concurrent Processing**: Process 10+ worksets simultaneously across multiple clusters
- **🔄 Intelligent Retry**: Automatic retry with exponential backoff and error classification
- **✅ Pre-execution Validation**: Catch configuration errors before consuming resources
- **👥 Multi-Tenant Support**: Customer portal with resource quotas and cost allocation
- **🔐 AWS Cognito Authentication**: Secure JWT-based authentication for customer access
- **📊 Real-time Monitoring**: REST API with queue statistics and workset tracking
- **🎯 Cluster Affinity**: Route worksets to optimal clusters based on data locality or cost
- **📢 Notifications**: Multi-channel notifications via SNS and Linear integration
...
# Start processor
processor = ConcurrentWorksetProcessor(state_db, scheduler, config)
processor.start()
```

### REST API Endpoints

The API can run with or without authentication:

**Without Authentication** (default - suitable for development/internal use):
```bash
# Install and run
pip install -e .
...
**With Authentication** (AWS Cognito - suitable for production):
```bash
...
Process multiple worksets in parallel with configurable concurrency limits:
...
#### Retry and Recovery
Intelligent retry system with error classification:
- **Transient errors** (network, throttling) → Automatic retry
- **Resource errors** (OOM, disk full) → Retry with backoff
- **Configuration errors** → No retry, immediate failure
- Exponential backoff: 2s, 4s, 8s, ... up to 1 hour
- Configurable max retry attempts per workset

#### Workset Validation
Pre-execution validation to catch errors early:
- Schema validation against JSON schema
- Reference genome availability checks
- FASTQ file existence verification
- Resource estimation (cost, duration, vCPU hours)
- Dry-run mode for testing

#### Customer Portal
Self-service portal for multi-tenant deployments:
- Automatic customer onboarding
- Dedicated S3 buckets per customer
- Resource quotas (concurrent worksets, storage)
...
- [Concurrent Processing Guide](docs/CONCURRENT_PROCESSING.md)
- [Customer Portal Guide](docs/CUSTOMER_PORTAL.md)
- [Workset Validation Guide](docs/WORKSET_VALIDATION.md)
- [Retry and Recovery Guide](docs/RETRY_RECOVERY.md)
