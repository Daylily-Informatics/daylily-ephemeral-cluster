...
REPORT = ROOT.parents[0] / "AWS_90day_cost_attribution_20260618.md"
LEDGER = ROOT.parents[0] / "plans" / "20260618T160028Z_aws_90day_cost_attribution_ledger.md"
...
def write_html_report(summary: dict[str, Any]) -> None:
    ...
    HTML_IMAGES.mkdir(parents=True, exist_ok=True)
    for image in IMAGES.glob("*.svg"):
        shutil.copy2(image, HTML_IMAGES / image.name)

    total = summary["total_cost"]
    cards = [
        ("Total 90-day spend", fmt_usd(total), "Unblended cost"),
        ("DYEC tagged", fmt_usd(summary["dyec_cluster_tagged_total"]), "cluster-name tag"),
        ("DYEC EC2 compute", fmt_usd(summary["dyec_ec2_compute"]), "tagged EC2 compute"),
        ("RDS/Aurora", fmt_usd(summary["rds_total"]), "service total"),
        ("Dayhoff/TapDB", fmt_usd(summary["dayhoff_lsmc_project_total"]), "lsmc-project tag"),
        ("Resource count", str(summary["resource_count"]), "live inventory"),
    ]
    cards_html = "\n".join(
        f'<div class="metric"><span class="label">{escape(label)}</span><strong>{escape(value)}</strong><span class="small">{escape(note)}</span></div>'
        for label, value, note in cards
    )
    category_rows = summary["category_surfaces"]
    html = f"""<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>AWS 90-Day Cost Attribution Report</title>
  <style>
    :root {{
      color-scheme: light;
      --bg: #f6f7f9;
    ...
    * {{ box-sizing: border-box; }}
    ...
    .page {{ width: min(1220px, calc(100% - 32px)); margin: 0 auto; padding: 28px 0 48px; }}
    header, .panel, .metric, .plot {{ background: var(--panel); border: 1px solid var(--line); border-radius: 8px; box-shadow: var(--shadow); }}
    header {{ padding: 24px; }}
    .eyebrow {{ margin: 0 0 6px; color: var(--blue); font-size: 0.82rem; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; }}
    h1 {{ margin: 0; font-size: clamp(2rem, 4vw, 3.1rem); line-height: 1.05; letter-spacing: 0; }}
    h2 {{ margin: 34px 0 14px; font-size: 1.45rem; line-height: 1.2; letter-spacing: 0; }}
    h3 {{ margin: 24px 0 10px; font-size: 1.05rem; line-height: 1.25; letter-spacing: 0; }}
    p {{ margin: 0 0 14px; }}
    code {{ background: #edf1f5; border: 1px solid #dce3eb; border-radius: 4px; padding: 1px 5px; font-family: ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; font-size: 0.92em; }}
    .meta {{ display: grid; grid-template-columns: repeat(5, minmax(150px, 1fr)); gap: 10px; margin-top: 20px; }}
    .meta div {{ background: var(--panel); border: 1px solid var(--line); border-radius: 8px; padding: 12px; box-shadow: var(--shadow); }}
    .label {{ display: block; color: var(--muted); font-size: 0.77rem; font-weight: 700; letter-spacing: 0.05em; text-transform: uppercase; }}
    .value {{ display: block; margin-top: 4px; font-weight: 700; overflow-wrap: anywhere; }}
    .notice {{ margin-top: 18px; padding: 12px 14px; border-left: 4px solid var(--amber); background: #fff8e7; color: #4a3511; }}
    .metrics {{ display: grid; grid-template-columns: repeat(6, minmax(140px, 1fr)); gap: 12px; margin: 18px 0 8px; }}
    .metric {{ padding: 14px; }}
    .metric strong {{ display: block; margin-top: 4px; font-size: 1.45rem; line-height: 1.1; }}
    .metric .small {{ color: var(--muted); font-size: 0.86rem; }}
    .panel {{ padding: 18px; margin-bottom: 16px; }}
    .plots {{ display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 14px; }}
    .plot {{ overflow: hidden; min-width: 0; }}
    .plot figcaption {{ padding: 12px 14px; color: var(--muted); font-weight: 700; border-bottom: 1px solid var(--line); }}
    .plot img {{ display: block; width: 100%; height: auto; background: #fff; }}
    table {{ width: 100%; border-collapse: collapse; margin: 10px 0 18px; font-size: 0.93rem; }}
    th, td {{ padding: 9px 10px; border-bottom: 1px solid var(--line); text-align: left; vertical-align: top; }}
    th {{ background: #eef3f8; font-size: 0.78rem; letter-spacing: 0.04em; text-transform: uppercase; color: #334155; }}
    tr:nth-child(even) td {{ background: #fafbfd; }}
    @media (max-width: 900px) {{
      .meta, .metrics, .plots {{ grid-template-columns: 1fr; }}
...
<body>
  <main class="page">
    <header>
      <p class="eyebrow">AWS Cost Explorer</p>
      <h1>AWS 90-Day Cost Attribution Report</h1>
      <div class="meta">
        <div><span class="label">Generated</span><span class="value">{escape(summary["generated_at"])}</span></div>
        <div><span class="label">Profile</span><span class="value">{PROFILE}</span></div>
        <div><span class="label">Account</span><span class="value">{ACCOUNT_ID}</span></div>
        <div><span class="label">Window</span><span class="value">{START} to 2026-06-18</span></div>
        <div><span class="label">Metric</span><span class="value">UnblendedCost</span></div>
      </div>
      <div class="notice">No AWS mutations were performed. Today, June 18, is included and may remain estimated until AWS billing settles.</div>
    </header>
    ...
    <section class="panel">
      <h2>Top Spend</h2>
      <h3>Services</h3>
      {table_html(summary["top_services"], [("Service", "name"), ("90-day spend", "amount_usd")], 8)}
      <h3>Regions</h3>
      {table_html(summary["top_regions"], [("Region", "name"), ("90-day spend", "amount_usd")], 8)}
    </section>

    <section>
      <h2>Overview Plots</h2>
      <div class="plots">
        <figure class="plot"><figcaption>Daily spend</figcaption><img src="images/daily_spend.svg" alt="Daily spend line chart"></figure>
        <figure class="plot"><figcaption>Top services</figcaption><img src="images/top_services.svg" alt="Top services bar chart"></figure>
        <figure class="plot"><figcaption>Top regions</figcaption><img src="images/top_regions.svg" alt="Top regions bar chart"></figure>
        <figure class="plot"><figcaption>Attribution surfaces</figcaption><img src="images/attribution_surfaces.svg" alt="Attribution surfaces bar chart"></figure>
      </div>
    </section>

    <section class="panel">
      <h2>Attribution Surfaces</h2>
      <p>These surfaces are not all mutually exclusive. Historical DYEC and Dayhoff/TapDB values use active billing tags; Terrarium and Aquarium use recent resource-level data and live inventory because historical app tags are not active.</p>
      {table_html(category_rows, [("Surface", "surface"), ("Evidence", "evidence"), ("90-day spend", "amount_usd"), ("Confidence", "confidence")])}
    </section>

    <section class="panel">
      <h2>DYEC / ParallelCluster</h2>
      {table_html(summary["dyec_cluster_service_breakdown"], [("Service", "name"), ("DYEC cluster-tagged spend", "amount_usd")], 12)}
      <h3>Top cluster-name tags</h3>
      {table_html(summary["top_dyec_clusters"], [("Cluster tag value", "name"), ("90-day spend", "amount_usd")], 15)}
    </section>

    <section class="panel">
      <h2>Dayhoff / TapDB</h2>
      {table_html(summary["dayhoff_lsmc_project_service_breakdown"], [("Service", "name"), ("Dayhoff/TapDB lsmc-project spend", "amount_usd")], 12)}
      <h3>Top lsmc-project values</h3>
      {table_html(summary["top_dayhoff_lsmc_projects"], [("lsmc-project value", "name"), ("90-day spend", "amount_usd")], 15)}
    </section>

    <section class="panel">
      <h2>Aurora / RDS</h2>
      <p>RDS/Aurora service total is {fmt_usd(summary["rds_total"])}. The apparent instance-class compute subset is {fmt_usd(summary["rds_compute_subset"])}.</p>
      {table_html(summary["rds_usage_type_breakdown"], [("RDS usage type", "name"), ("90-day spend", "amount_usd")], 15)}
    </section>

    <section class="panel">
      <h2>Terrarium And Aquarium</h2>
      {table_html(summary["recent_resource_costs_by_class"], [("Class", "name"), ("Recent resource-level spend", "amount_usd"), ("30.4-day run-rate", "monthly_run_rate_usd")])}
      <h3>Recent class/service split</h3>
      {table_html(summary["recent_resource_costs_by_class_service"], [("Class", "classification"), ("Service", "service"), ("Recent spend", "amount_usd"), ("30.4-day run-rate", "monthly_run_rate_usd")], 20)}
      <h3>Live resource counts</h3>
      {table_html(summary["live_resource_counts_by_class"], [("Class", "name"), ("Live resources", "count")])}
    </section>

    <section class="panel">
      <h2>Limits And Reproducibility</h2>
      <p>{escape(summary["attribution_limits"])}</p>
      <p>Collector command records: {summary["command_count"]}; expected S3 metadata misses: {summary["expected_s3_no_lifecycle_count"]} lifecycle and {summary["expected_s3_no_tag_set_count"]} tag-set responses; unexpected command errors: {summary["unexpected_command_error_count"]}.</p>
    </section>
  </main>
</body>
</html>
...
AWS profile: `{PROFILE}`
Account: `{ACCOUNT_ID}`
Window: {START} through 2026-06-18 inclusive (`Cost Explorer End={END}`)
...
Recent resource-level Cost Explorer data for {RECENT_START} through 2026-06-18 provides a partial actual-spend signal:
...
def main() -> None:
    ...
    summary = {
        ...
        "window": {"start": START, "end_exclusive": END, "last_inclusive_day": "2026-06-18"},
