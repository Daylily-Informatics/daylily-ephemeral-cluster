...
This document is the operator-facing reference for the `18.0.13` `dyec` command surface. It favors explicit commands and receipts over implicit state. `daylily-ec` is an installed compatibility entrypoint for the same CLI, but current docs and ledgers use `dyec`.
...
The active catalog targets DayOA `15.0.7`. `validation_pending: true` means a
