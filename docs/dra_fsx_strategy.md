...
`config/daylily_pipeline_command_catalog.yaml` defines repositories and launch profiles. The DayOA repository and every DayOA command are pinned to `2.0.41`.
...
```mermaid
flowchart TB
  Catalog["Repository catalog v2"] --> Repo["daylily-omics-analysis @ 2.0.41"]
