# DYEC 10.0.63 Release Evidence

Generated: in progress, opened 2026-06-20T03:05Z.

## Scope

This report tracks the DYEC `10.0.63` release validation on `tstpartition` in `us-west-2` using AWS profile `lsmc`. The DayOA catalog pin under evidence is `10.0.34`.
...
|---|---:|---|---|
| `daylily-omics-analysis` | `10.0.34` | `aa9af67` | Annotated release tag on `jem-dev`; BCL FastQC moved-output symlink fix. |
| `daylily-ephemeral-cluster` | `10.0.63` | `4b336d41` | Annotated release tag on `jem-dev`; DYEC self-pin to `10.0.62`. |
...
| `ont_run_qc` | PENDING | pending | pending export | Requires ONT run DRA after current Illumina sequence reaches a terminal state. |
...
- Run `ont_run_qc` and `ultima_run_qc` one at a time.
