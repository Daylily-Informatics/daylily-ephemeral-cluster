...
|---|---|---|---|---|
| DayOA | `daylily-omics-analysis` | Snakemake 7 through `dy-r` | Current DYEC catalog command `illumina_snv_alignstats`, DayOA `2.0.44`, genome `hg38_broad`, `-j 20` | Launched as `dyec5128_hg003_5x_ilmn_snv_20260603T110935Z`; terminal metrics pending |
...
- Current catalog validation came from `config/daylily_pipeline_command_catalog.yaml`, not an old plan. `illumina_snv_alignstats` is DayOA `2.0.44` with targets `produce_sent_align`, `produce_dmd_dedup_cram`, `produce_sentd_snv_vcf`, `produce_snv_concordances`, and `produce_alignstats`.
