...
| LIVE-001 | Live run | If and only if dry-run gate passes, run exact command without `-n` in the same remote tmux shell. | BLOCKED | feature_implementation | Gate 3 | orchestrator | Not launched. The last corrected dry-run (`DRY-005`) exited `0`, but `GATE-005` failed because forbidden heavy rules were planned. | Dry-run safety gate failed after the `lsmc-bio` MultiQC correction. | Exact live command remains blocked by Gate 2. |
| ACCEPT-001 | Acceptance | Verify workflow success, empty queue for run, final MultiQC, expected rollups, metagenomics output, and no VerifyBamID2. | BLOCKED | contract_test | Gate 5 | orchestrator | Not run because live workflow was not launched. Post-dry-run status at `2026-05-28T06:38Z` showed an empty Slurm queue and both tmux sessions preserved. | Dry-run safety gate failed after the `lsmc-bio` MultiQC correction. | Acceptance cannot be evaluated until a dry-run gate passes and a live run is allowed and completes. |
...
| MQC-001 | MultiQC runtime | Patch the remote DayOA run checkout so MultiQC rules use the DayOA-pinned `lsmc-bio` MultiQC conda env (`1.36.dev0-lsmc.6`) instead of `docker://multiqc/multiqc:v1.35`. | SUCCESS | plan_amendment | Gate 2 | orchestrator | Remote inspection at `2026-05-28T06:33Z` confirmed `workflow/envs/multiqc_v0.1.yaml` pins `lsmc-bio/MultiQC` tag `1.36.dev0-lsmc.6`, while report rules still hardcoded `docker://multiqc/multiqc:v1.35`. Patched seven remote rule files: `expansionhunter.smk`, `multiqc_cov_aln.smk`, `multiqc_final_wgs.smk`, `multiqc_for_bcl2fq.smk`, `multiqc_for_raw_fastqs.smk`, `multiqc_singleton.smk`, and `unmapped_metagenomics.smk`; backups were written as `*.bak.lsmc_multiqc_20260528T0635Z`. | User amended the runtime contract; the hardcoded container bypassed the required hacked MultiQC release. | The target-relevant MultiQC rules now use `conda: config["multiqc"].get("env_yaml", "../envs/multiqc_v0.1.yaml")` with `container: None`; the only remaining upstream string is a non-executed metadata reference in `evidence_manifest.smk`, and `write_dayoa_evidence_manifest` was not in the corrected dry-run. |
...
| DRY-005 | Dry-run retry with lsmc-bio MultiQC | Retry `multiqc_final_wgs` no-evidence dry-run after the remote run checkout is corrected to use the `lsmc-bio` MultiQC env. | SUCCESS | plan_amendment | Gate 2 | orchestrator | Ran in tmux session `inflection_20x10x_kitchensink_j250_20260528T043733Z`; marker `__DAYOA_DRYRUN_LSMC_MQC_DONE_RC_0__` at `2026-05-28T06:36:42Z`. The dry-run used `multiqc_final_wgs` instead of `produce_multiqc_all`, included Ganon2 and Kraken2 durable `/fsx/references/...` DB paths, exited `0`, did not include `write_dayoa_evidence_manifest`, did not include VerifyBamID2, and showed only `ILMN20X-ONT10X` / `ONT10X` scope. | User requested not to run `write_dayoa_evidence_manifest` and then amended the MultiQC runtime requirement. | Dry-run execution succeeded, but the safety gate failed separately in `GATE-005`. |
| GATE-005 | Retry gate with lsmc-bio MultiQC | Evaluate the `lsmc-bio` MultiQC dry-run. Live remains blocked unless the retry exits `0` and proves no align/dedup/variant/segdup/CNV/mito/EH/VB2/VerifyBamID2 reruns. | BLOCKED | legitimate_safety_handling | Gate 2 | orchestrator | Corrected dry-run exited `0`, requested downstream/report work was present, `write_dayoa_evidence_manifest` and VerifyBamID2 were absent, and scope was only `ILMN20X-ONT10X`; however the planned job table included forbidden heavy rules: `sentieon_bwa_sort` (`1`), `doppelmark_dups` (`1`), `expansionhunter_call` (`1`), and `expansionhunter_json_to_tsv` (`1`). The table also showed `141` total planned jobs. | Gate condition forbidding alignment, dedup, and ExpansionHunter execution failed; Snakemake reported missing outputs/input-updated-by-another-job paths for the active 20x/10x sample under DayOA `2.0.13`. | Live execution was not launched. |
...
- `2026-05-28T06:33Z`: user amended the runtime requirement to use the hacked `lsmc-bio` MultiQC release with Ultima run-directory QC and first-class AlignStats support. Remote inspection confirmed `workflow/envs/multiqc_v0.1.yaml` already pins `multiqc @ https://github.com/lsmc-bio/MultiQC/archive/refs/tags/1.36.dev0-lsmc.6.zip`, but seven MultiQC rule files still forced `docker://multiqc/multiqc:v1.35`.
- `2026-05-28T06:35Z`: patched the remote DayOA run checkout so target-relevant MultiQC rules use the pinned `lsmc-bio` conda env and no container. Backups were written as `*.bak.lsmc_multiqc_20260528T0635Z`.
- `2026-05-28T06:36Z`: corrected `lsmc-bio` MultiQC dry-run with `multiqc_final_wgs` exited `0`; it did not plan `write_dayoa_evidence_manifest` or VerifyBamID2, and it stayed scoped to `ILMN20X-ONT10X` / `ONT10X`.
- `2026-05-28T06:38Z`: dry-run gate failed despite rc `0` because forbidden heavy rules were planned: `sentieon_bwa_sort`, `doppelmark_dups`, `expansionhunter_call`, and `expansionhunter_json_to_tsv`. Slurm queue was empty after the dry-run; no live workflow was launched.

## Terminal Status

- Ledger rows are terminal: `G0-001`, `DB-001`, `DB-002`, `GANON2-001`, `VER-001`, `CFG-001`, `MQC-001`, and `DRY-005` are `SUCCESS`; `DRY-001`, `DRY-002`, and `DRY-003` are `FAIL`; `CONT-001`, `DRY-004`, and `GATE-004` are `NO_LONGER_NEEDED`; `GATE-001`, `GATE-002`, `GATE-005`, `LIVE-001`, and `ACCEPT-001` are `BLOCKED`.
- Workflow objective did not complete. The corrected `lsmc-bio` MultiQC dry-run exited `0`, but the safety gate failed because alignment, dedup, and ExpansionHunter execution were planned. The exact live command was not launched.
