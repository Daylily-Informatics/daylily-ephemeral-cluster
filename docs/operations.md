...
This guide describes the `18.0.17` CLI. For repeated work, `dyec set-vars` can
...
The active catalog pin for DayOA commands is `15.0.9`. A catalog row can retain
...
```bash
day-clone --list
day-clone --repository daylily-omics-analysis --destination "$ANALYSIS_ID" --git-tag 15.0.9 --executing-entity "$EXECUTING_ENTITY"
day-clone -d "$ANALYSIS_ID" -t 15.0.9
```
