...
The active catalog pin for DayOA commands is `15.0.11`. A catalog row can retain
...
```bash
day-clone --list
day-clone --repository daylily-omics-analysis --destination "$ANALYSIS_ID" --git-tag 15.0.11 --executing-entity "$EXECUTING_ENTITY"
day-clone -d "$ANALYSIS_ID" -t 15.0.11
```
