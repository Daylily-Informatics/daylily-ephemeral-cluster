...
| `FAIL-FIRST-001` | Workflow | Run/fix `hybrid_ultima_ont_snv` using only `sentdhuomr`. | `IN_PROGRESS` | `feature_implementation` | Gate 5 | Workflow Agent | Prior failure: `ModuleNotFoundError: No module named 'sentieon_cli'` in `sentdhuomr_hybrid_select`. Dry-run launched from pushed DayOA branch `codex/tstclu411c-hybrid-env-python` with command `bin/day_run produce_sentdhuomr_snv_vcf produce_alignstats produce_snv_concordances -p -j 100 -k -n`, analysis/session `tcc_hybrid_ultima_ont_snv_5x5x_fix_dryrun`, stage dir `/fsx/data/staged_sample_data/remote_stage_20260526T062121Z`. Current status: `exit_code=null`; log shows conda environment transaction in progress. | Original root cause likely bare base `python` resolving outside the rule env; cluster dry-run still validating fix. |  |
