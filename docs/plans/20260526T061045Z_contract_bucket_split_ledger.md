...
- Sweep: `rg -n "/fsx/data|/data/staged_sample_data|reference-bucket|reference_bucket|s3_bucket_name|REGSUB_S3_BUCKET" daylily_ec config tests docs README.md pyproject.toml | wc -l` -> `1282`
- DayOA sweep: `rg -n "/fsx/data|/data/staged_sample_data|reference-bucket|reference_bucket|genomic_data|cached_envs|staged_sample_data" README.md dyoainit config workflow tests docs bin | wc -l` -> `657`
...
- Assumption: active contracts move to `/fsx/references`, `/fsx/control_data`, `/fsx/runtime_assets`, and `/fsx/staging`; no `/fsx/data` compatibility mount or fallback bucket discovery is allowed.
...
| DYEC-004 | Setup/readiness | Update setup script and headnode readiness from `/fsx/data` to role roots. | OPEN | feature_implementation | Gate 2 | Agent B |  |  |  |
...
| DYEC-006 | Staging CLI | Move samples staging to explicit role buckets and default `/fsx/staging/staged_sample_data`; reject `/data` and `/fsx/data`. | OPEN | feature_implementation | Gate 2 | Agent C |  |  |  |
...
| DYEC-009 | Docs | Update DayEC docs/examples away from overloaded reference bucket and `/fsx/data`. | OPEN | feature_implementation | Gate 5 | Agent E |  |  |  |
