...
| REL-002 | DYEC release A | Update source and packaged DayOA catalog pins from `2.0.2` to `2.0.3`, include Goodole3 plan artifacts, validate parity/tests, commit, push, annotated-tag `5.0.3`, and push tag. | IN_PROGRESS | config_or_startup_contract | Gate 2 | Agent 2 + orchestrator | Source and packaged catalogs, README, docs, and catalog tests changed to `2.0.3`; catalog `cmp` passed; focused DYEC tests passed `234 passed`; `git diff --check` passed. Commit/tag pending. |  |  |
| REL-003 | DYEC release B | Update source and packaged self-pins from `5.0.2` to final `5.0.4`, validate parity/tests/ruff/diff-check, commit, push, annotated-tag `5.0.4`, and push tag. | OPEN | config_or_startup_contract | Gate 3 | Agent 2 + orchestrator | Current source and packaged self-pins are `5.0.2`; update pending Release A. |  |  |
