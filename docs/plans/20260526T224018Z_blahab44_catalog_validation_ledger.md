...
## 2026-05-27T16:17Z VerifyBamID2 Cancellation And Resume

- User provided the required second explicit approval with `scancel 48 185 on blahab44` and directed the running Snakemake controllers to continue after removing VerifyBamID2 commands from active execution.
- Executed `scancel 48 185` on `blahab44` through DAY-EC SSM helper access as `ubuntu`; evidence log: `docs/plans/20260526T224018Z_blahab44_logs/blahab44_scancel_verifybamid2_48_185_20260527T162000Z.log`.
- The ONT controller requeued one additional VerifyBamID2 Slurm job, `248`; that requeued job was cancelled under the same user direction to remove VerifyBamID2 commands from running. Evidence log: `docs/plans/20260526T224018Z_blahab44_logs/blahab44_scancel_verifybamid2_requeue_20260527T162400Z.log`.
- The original ONT and hybrid kitchensink controllers exited after the cancelled external VerifyBamID2 jobs, as expected for the old active DayOA `2.0.5` Snakemake include.
- The two live analysis clones were hot-upgraded in place to DayOA `2.0.8`: `/fsx/analysis_results/ubuntu/bb44_ont_snv_alignstats_kitchensink/daylily-omics-analysis` and `/fsx/analysis_results/ubuntu/bb44_hybrid_ilmn_ont_snv_kitchensink/daylily-omics-analysis`. Both checked out `5cc2200` (`tag: 2.0.8`) and both Snakefiles show `# include: "rules/verifybamid2_contam.smk"` retired from active Snakemake.
- Resume scripts were written under `/home/ubuntu/daylily-runs/.../resume_no_verifybamid2_2.0.8.sh` and launched in the existing tmux sessions. Evidence log: `docs/plans/20260526T224018Z_blahab44_logs/blahab44_resume_without_verifybamid2_20260527T163000Z.log`.
- First resume attempts exposed active generated profile drift: `day_activate` freshness prompt and then `KeyError: 'ngstroublefinder'` from old `2.0.5` generated profile config. Active generated `config/day_profiles/slurm` files were backed up under each clone's `pre_2.0.8_resume_backup_20260527T163600Z` directory and refreshed from the DayOA `2.0.8` templates. Evidence log: `docs/plans/20260526T224018Z_blahab44_logs/blahab44_refresh_dayoa_208_profile_configs_20260527T163600Z.log`.
- Latest monitor at `2026-05-27T16:38:08Z` showed `squeue` empty, `VERIFYBAMID2_COUNT=0`, and both resumed controller stacks still running. Evidence logs: `docs/plans/20260526T224018Z_blahab44_logs/bb44_ont_snv_alignstats_kitchensink_resume_status_20260527T163704Z.json`, `docs/plans/20260526T224018Z_blahab44_logs/bb44_ont_snv_alignstats_kitchensink_resume_tmux_tail_20260527T163704Z.log`, `docs/plans/20260526T224018Z_blahab44_logs/bb44_hybrid_ilmn_ont_snv_kitchensink_resume_status_20260527T163704Z.json`, `docs/plans/20260526T224018Z_blahab44_logs/bb44_hybrid_ilmn_ont_snv_kitchensink_resume_tmux_tail_20260527T163704Z.log`, and `docs/plans/20260526T224018Z_blahab44_logs/blahab44_slurm_resume_20260527T163800Z.log`.
- Final quick persisted check at `2026-05-27T16:41:55Z` again showed empty `squeue`, `VERIFYBAMID2_COUNT=0`, and `CONTROLLER_COUNT=8`; both workflow status payloads still had `exit_code=null` and `completed_at=null`. Evidence logs: `docs/plans/20260526T224018Z_blahab44_logs/bb44_ont_snv_alignstats_kitchensink_resume_status_20260527T164100Z.json`, `docs/plans/20260526T224018Z_blahab44_logs/bb44_hybrid_ilmn_ont_snv_kitchensink_resume_status_20260527T164100Z.json`, and `docs/plans/20260526T224018Z_blahab44_logs/blahab44_slurm_resume_20260527T164100Z.log`.

## Heartbeat 2026-05-27T16:44Z Monitoring

- Re-read repo instructions, plan-ledger SOP, current ledger, and git state. Repo branch is `codex/running-nextflow-pipes-doc` at `21219791`; unrelated GoodOle3/HG003/report files remain dirty or untracked and were not touched.
- AWS identity evidence before the live status check: `AWS_PROFILE=lsmc`, region `us-west-2`, account `108782052779`, ARN `arn:aws:iam::108782052779:root`.
- Workflow status/log sweep appended to `docs/plans/20260526T224018Z_blahab44_catalog_runs.jsonl` and wrote:
  - `docs/plans/20260526T224018Z_blahab44_logs/bb44_ont_snv_alignstats_kitchensink_heartbeat_status_20260527T164457Z.json`
  - `docs/plans/20260526T224018Z_blahab44_logs/bb44_ont_snv_alignstats_kitchensink_heartbeat_tmux_tail_20260527T164457Z.log`
  - `docs/plans/20260526T224018Z_blahab44_logs/bb44_hybrid_ilmn_ont_snv_kitchensink_heartbeat_status_20260527T164457Z.json`
  - `docs/plans/20260526T224018Z_blahab44_logs/bb44_hybrid_ilmn_ont_snv_kitchensink_heartbeat_tmux_tail_20260527T164457Z.log`
  - `docs/plans/20260526T224018Z_blahab44_logs/blahab44_slurm_results_heartbeat_20260527T164457Z.log`
- Remaining non-terminal sessions: `bb44_ont_snv_alignstats_kitchensink` and `bb44_hybrid_ilmn_ont_snv_kitchensink`, both with `exit_code=null`, `completed_at=null`, and resumed `started_at=2026-05-27T16:30:31Z`.
- Headnode Slurm evidence via `daylily_ec.aws.ssm.run_shell` as `ubuntu` on `ip-10-0-0-224` at `2026-05-27T16:45:39Z`: `squeue` empty, `VERIFYBAMID2_COUNT=0`, and `CONTROLLER_COUNT=8`; both Snakemake controller process stacks remained active.
- Both live clones remained at DayOA `2.0.8` commit `5cc2200`; both Snakefiles showed the retired commented `verifybamid2_contam.smk` include only.
- Result checks: ONT result root `/fsx/analysis_results/ubuntu/bb44_ont_snv_alignstats_kitchensink/daylily-omics-analysis/results` had `done_count=7`; Hybrid result root `/fsx/analysis_results/ubuntu/bb44_hybrid_ilmn_ont_snv_kitchensink/daylily-omics-analysis/results` had `done_count=11`; both were still missing `results/day/hg38_broad/reports/DAY_final_multiqc.html`, `results/day/hg38_broad/reports/DAY_final_multiqc_data`, `results/day/hg38_broad/other_reports/giab_concordance_mqc.tsv`, and `results/day/hg38_broad/other_reports/vep_annotation_mqc.tsv`.
- ONT tmux tail showed env creation progressed through `gatkcontam`, `mosdepth`, `rtgtools`, `go_left`, `alignstats`, `vanilla`, `site_mix_contam`, and was creating `sentieon_v0.3`; Hybrid tmux tail showed env creation progressed through `fastqc`, `samtools`, and was creating `peddy_v0.1`.

## 2026-05-27T17:13Z Kill/Rerun Preflight

- User requested killing VerifyBamID2 jobs, killing the two Snakemake controllers, disabling VB2, then validating `--rerun-triggers mtime -n` before live rerun. This is a destructive live compute interruption, so only preflight was performed pending the required second explicit confirmation.
- Re-read repo/global instructions, plan-ledger SOP, current ledger, and git state. Repo branch is `codex/running-nextflow-pipes-doc` at `21219791`; unrelated GoodOle3/HG003/report files remain dirty or untracked and were not touched.
- AWS identity evidence before live inspection: `AWS_PROFILE=lsmc`, region `us-west-2`, account `108782052779`, ARN `arn:aws:iam::108782052779:root`.
- Non-destructive DAY-EC SSM helper preflight wrote:
  - `docs/plans/20260526T224018Z_blahab44_logs/blahab44_pre_kill_verifybamid_controllers_20260527T171329Z.log`
  - `docs/plans/20260526T224018Z_blahab44_logs/blahab44_pre_kill_corrected_jobs_20260527T171450Z.log`
  - `docs/plans/20260526T224018Z_blahab44_logs/bb44_ont_snv_alignstats_kitchensink_pre_kill_status_20260527T171329Z.json`
  - `docs/plans/20260526T224018Z_blahab44_logs/bb44_ont_snv_alignstats_kitchensink_pre_kill_tmux_tail_20260527T171329Z.log`
  - `docs/plans/20260526T224018Z_blahab44_logs/bb44_hybrid_ilmn_ont_snv_kitchensink_pre_kill_status_20260527T171329Z.json`
  - `docs/plans/20260526T224018Z_blahab44_logs/bb44_hybrid_ilmn_ont_snv_kitchensink_pre_kill_tmux_tail_20260527T171329Z.log`
- Preflight found no active VerifyBamID2 Slurm jobs: `VERIFYBAMID2_COUNT=0`.
- Both target tmux sessions have exactly one window and one pane: `bb44_ont_snv_alignstats_kitchensink|windows=1|panes=1` and `bb44_hybrid_ilmn_ont_snv_kitchensink|windows=1|panes=1`.
- Controller root PIDs: `873572` for ONT resume script and `873580` for Hybrid resume script. Snakemake PIDs: `874151` for ONT and `874153` for Hybrid.
- Current target-session Slurm jobs that would become orphaned if the controllers are killed without cancelling them: `367 352 351 353 354 355 356 357 358 359 360 361 362 363 364 365 366 371 372 373`.
- Both live clones remained at DayOA `2.0.8` commit `5cc2200`; both Snakefiles showed only the commented retired `verifybamid2_contam.smk` include. No destructive action was executed; live cancellation is blocked pending second confirmation.

## Heartbeat 2026-05-27T17:46Z Monitoring

- Re-read repo instructions, plan-ledger SOP, current ledger, and git state. Repo branch is `codex/running-nextflow-pipes-doc` at `21219791`; unrelated GoodOle3/HG003/report files remain dirty or untracked and were not touched.
- The kill/rerun path remains blocked because no second explicit confirmation arrived after the `2026-05-27T17:13Z` destructive preflight; this pass performed read-only monitoring only.
- AWS identity evidence before the live status check: `AWS_PROFILE=lsmc`, region `us-west-2`, account `108782052779`, ARN `arn:aws:iam::108782052779:root`.
- Workflow status/log sweep appended to `docs/plans/20260526T224018Z_blahab44_catalog_runs.jsonl` and wrote:
  - `docs/plans/20260526T224018Z_blahab44_logs/bb44_ont_snv_alignstats_kitchensink_heartbeat_status_20260527T174646Z.json`
  - `docs/plans/20260526T224018Z_blahab44_logs/bb44_ont_snv_alignstats_kitchensink_heartbeat_tmux_tail_20260527T174646Z.log`
  - `docs/plans/20260526T224018Z_blahab44_logs/bb44_hybrid_ilmn_ont_snv_kitchensink_heartbeat_status_20260527T174646Z.json`
  - `docs/plans/20260526T224018Z_blahab44_logs/bb44_hybrid_ilmn_ont_snv_kitchensink_heartbeat_tmux_tail_20260527T174646Z.log`
  - `docs/plans/20260526T224018Z_blahab44_logs/blahab44_readonly_heartbeat_20260527T174646Z.log`
- Remaining non-terminal sessions: `bb44_ont_snv_alignstats_kitchensink` and `bb44_hybrid_ilmn_ont_snv_kitchensink`, both with `exit_code=null`, `completed_at=null`, and resumed `started_at=2026-05-27T16:30:31Z`.
- Headnode Slurm evidence via `daylily_ec.aws.ssm.run_shell` as `ubuntu` on `ip-10-0-0-224` at `2026-05-27T17:47:29Z`: `VERIFYBAMID_COUNT=0`, `CONTROLLER_COUNT=8`, and target-session jobs were active/pending for ONT concordance/VEP finalization and Hybrid `sentdhiomr_stage3`/`sentdhiomr_pass2` work.
- ONT progress: tmux tail showed `70 of 84 steps (83%) done`; result root `/fsx/analysis_results/ubuntu/bb44_ont_snv_alignstats_kitchensink/daylily-omics-analysis/results` had `done_count=9`, `vep_annotation_mqc.tsv` existed, and final MultiQC/report/GIAB concordance paths were still missing.
- Hybrid progress: tmux tail showed `158 of 810 steps (20%) done`; result root `/fsx/analysis_results/ubuntu/bb44_hybrid_ilmn_ont_snv_kitchensink/daylily-omics-analysis/results` had `done_count=11`, and final MultiQC/GIAB/VEP report paths were still missing.
...
| 6 | `ont_snv_alignstats_kitchensink` | sample_analysis | `2.0.5` original, resumed on `2.0.8` | RUNNING | Forced dry-run `bb44_ont_snv_alignstats_kitchensink_dryrun_retry2` exit `0`; original live run started `2026-05-27T02:07:09Z`; user-approved `scancel 48` removed the active VerifyBamID2 Slurm job and requeued VerifyBamID2 job `248` was also cancelled; clone hot-upgraded to DayOA `2.0.8` at `5cc2200`, with `verifybamid2_contam.smk` commented out; resumed no-VerifyBamID2 controller started `2026-05-27T16:30:31Z` and remained non-terminal at `2026-05-27T17:47:29Z`; repo path `/fsx/analysis_results/ubuntu/bb44_ont_snv_alignstats_kitchensink/daylily-omics-analysis`; result root `/fsx/analysis_results/ubuntu/bb44_ont_snv_alignstats_kitchensink/daylily-omics-analysis/results`; latest status/tmux logs are `bb44_ont_snv_alignstats_kitchensink_heartbeat_status_20260527T174646Z.json` and `bb44_ont_snv_alignstats_kitchensink_heartbeat_tmux_tail_20260527T174646Z.log`; SSM evidence `blahab44_readonly_heartbeat_20260527T174646Z.log` showed `VERIFYBAMID_COUNT=0`, active/pending ONT concordance/VEP jobs, `done_count=9`, and `vep_annotation_mqc.tsv` present while final MultiQC and GIAB concordance outputs remained missing. |
...
| 10 | `hybrid_ilmn_ont_snv_kitchensink` | sample_analysis | `2.0.5` original, resumed on `2.0.8` | RUNNING | Forced dry-run `bb44_hybrid_ilmn_ont_snv_kitchensink_dryrun_retry2` exit `0`; original live run started `2026-05-27T02:12:02Z`; user-approved `scancel 185` removed the active VerifyBamID2 Slurm job; clone hot-upgraded to DayOA `2.0.8` at `5cc2200`, with `verifybamid2_contam.smk` commented out; resumed no-VerifyBamID2 controller started `2026-05-27T16:30:31Z` and remained non-terminal at `2026-05-27T17:47:29Z`; repo path `/fsx/analysis_results/ubuntu/bb44_hybrid_ilmn_ont_snv_kitchensink/daylily-omics-analysis`; result root `/fsx/analysis_results/ubuntu/bb44_hybrid_ilmn_ont_snv_kitchensink/daylily-omics-analysis/results`; latest status/tmux logs are `bb44_hybrid_ilmn_ont_snv_kitchensink_heartbeat_status_20260527T174646Z.json` and `bb44_hybrid_ilmn_ont_snv_kitchensink_heartbeat_tmux_tail_20260527T174646Z.log`; SSM evidence `blahab44_readonly_heartbeat_20260527T174646Z.log` showed `VERIFYBAMID_COUNT=0`, active/pending Hybrid `sentdhiomr_stage3`/`sentdhiomr_pass2` jobs, `done_count=11`, and final MultiQC/GIAB/VEP outputs still missing. |
