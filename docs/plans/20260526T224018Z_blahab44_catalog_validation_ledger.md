...
| 4 | `ultima_snv_alignstats_kitchensink` | sample_analysis | `2.0.5` | RUNNING | Forced dry-run `bb44_ultima_snv_alignstats_kitchensink_dryrun_retry2` exit `0`; live `bb44_ultima_snv_alignstats_kitchensink_retry2` started `2026-05-27T02:03:56Z` and was non-terminal at `2026-05-27T02:43Z`; repo path `/fsx/analysis_results/ubuntu/bb44_ultima_snv_alignstats_kitchensink_retry2/daylily-omics-analysis`; result root `/fsx/analysis_results/ubuntu/bb44_ultima_snv_alignstats_kitchensink_retry2/daylily-omics-analysis/results`; tmux tail showed `98 of 111 steps (88%) done`. |
...
| 6 | `ont_snv_alignstats_kitchensink` | sample_analysis | `2.0.5` | RUNNING | Forced dry-run `bb44_ont_snv_alignstats_kitchensink_dryrun_retry2` exit `0`; live `bb44_ont_snv_alignstats_kitchensink` started `2026-05-27T02:07:09Z` and was non-terminal at `2026-05-27T02:43Z`; repo path `/fsx/analysis_results/ubuntu/bb44_ont_snv_alignstats_kitchensink/daylily-omics-analysis`; result root `/fsx/analysis_results/ubuntu/bb44_ont_snv_alignstats_kitchensink/daylily-omics-analysis/results`; tmux tail showed `23 of 106 steps (22%) done`. |
...
| 10 | `hybrid_ilmn_ont_snv_kitchensink` | sample_analysis | `2.0.5` | RUNNING | Forced dry-run `bb44_hybrid_ilmn_ont_snv_kitchensink_dryrun_retry2` exit `0`; live `bb44_hybrid_ilmn_ont_snv_kitchensink` started `2026-05-27T02:12:02Z` and was non-terminal at `2026-05-27T02:43Z`; repo path `/fsx/analysis_results/ubuntu/bb44_hybrid_ilmn_ont_snv_kitchensink/daylily-omics-analysis`; result root `/fsx/analysis_results/ubuntu/bb44_hybrid_ilmn_ont_snv_kitchensink/daylily-omics-analysis/results`; tmux tail showed `37 of 848 steps (4%) done`. |
...
| CAT-001 | Catalog launch | Run all 17 DayOA command catalog commands against `blahab44`; each command row must reach `SUCCESS`, `FAIL`, `RUNNING`, or `BLOCKED` with exact dry-run/live evidence. | IN_PROGRESS | contract_test | Gate 3 | orchestrator | Event stream `docs/plans/20260526T224018Z_blahab44_catalog_runs.jsonl`; logs `docs/plans/20260526T224018Z_blahab44_logs/`; as of the `2026-05-27T02:43Z` status sweep: 2 live successes, 10 live failures, 3 live kitchensink rows still running, 1 dry-run failure, and 1 blocked command. Slurm evidence after live pass showed active jobs as `ubuntu`, including contamination jobs for the running kitchensinks. | Post-fix live rows no longer fail on read-only reference conda path; residual failures are now command/runtime-specific or shared writable conda-cache concurrency issues. | Not terminal yet because three kitchensink rows are still running. No full kitchensink live run has completed successfully yet. |
