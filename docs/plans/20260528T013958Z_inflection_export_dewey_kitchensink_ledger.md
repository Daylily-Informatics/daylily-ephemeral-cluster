...
- Local branch/status at Gate 0: `codex/running-nextflow-pipes-doc...origin/codex/running-nextflow-pipes-doc`, with pre-existing modified/untracked plan artifacts, `AGENTS.md`, `.tailscale_key`, and unrelated local VCF/presign files.
...
- 2026-05-28: The 20x10 kitchen-sink/QEO controller was not restarted because the run lacks explicit `qeo_registration` identities and `goodole3` is not receiving tailnet split DNS for `day.lsmc.bio`.
- 2026-05-28: Read-only network debug showed `goodole3` has general outbound internet: Google returned HTTP 204, AWS returned HTTP 200, S3 and STS endpoints returned redirects, and `aws sts get-caller-identity` succeeded. The Dewey failure is narrower: `tailscale dns status` reports Tailscale DNS disabled even though split-DNS route `day.lsmc.bio -> 100.75.231.53` is advertised; public `dewey.day.lsmc.bio` resolves to `52.89.110.76`, but the Dayhoff public service SG only allows TCP 443 from `152.44.152.3/32` and `108.212.69.87/32`, while `goodole3` egresses as `44.242.81.64`.
