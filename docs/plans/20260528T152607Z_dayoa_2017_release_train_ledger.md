...
| RT-006 | DYEC DayOA pin | Update DYEC env/config/pyproject DayOA pins to the new DayOA release, commit/push/tag | SUCCESS | feature_implementation | Gate 5 | orchestrator | DayOA pins updated in source and packaged catalogs plus docs/tests to `2.0.19`; `environment.yaml` and `pyproject.toml` do not carry a DayOA package dependency/pin in this repo, so no DayOA dependency edit was made there. Commit `66b6ecf565777eb079b714da50e1df21db13dc63` pushed to `origin/codex/dyec-dewey-registration-refactor-20260528`; annotated tag `5.0.21` created (`git cat-file -t 5.0.21 -> tag`) and pushed. |  | DYEC `5.0.21` pins DayOA `2.0.19` and includes the kitchen-sink catalog evidence. |
| RT-007 | DYEC self pin | Update DYEC self pin in config, commit/push/tag final version | SUCCESS | feature_implementation | Gate 5 | orchestrator | Source and packaged `daylily_cli_global.yaml` updated to self-pin `git_ephemeral_cluster_repo_tag` and `git_ephemeral_cluster_repo_release_tag` to `5.0.21`. Validation: `python -m pytest -q tests/test_packaged_defaults.py tests/test_versioning.py tests/test_repository_catalog.py` -> `20 passed`; `git diff --check` clean. |  | This final self-pin commit is the intended DYEC `5.0.22` release commit. |
