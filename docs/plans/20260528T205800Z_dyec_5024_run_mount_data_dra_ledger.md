...
| DYEC28-004 | Orchestrator | Commit, tag `5.0.28`, build, publish, and verify package availability. | SUCCESS | 3 | Commit `ea58ba5d`; annotated tag `5.0.28`; branch, main, and tag pushed; `twup` uploaded artifacts; PyPI reports latest `5.0.28`. | Ursa must pin `daylily-ephemeral-cluster==5.0.28` before the next production retry. |
