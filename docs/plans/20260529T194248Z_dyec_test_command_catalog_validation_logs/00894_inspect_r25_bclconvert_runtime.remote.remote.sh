set -euo pipefail
aid=ccv20260529r25_illumina_bclconvert
...
echo
echo "run_log_tail:"
tail -n 160 "$repo/results/runs/20260514_LH01106_0009_B23TVLGLT4/bclconvert/logs/run_bclconvert.log" 2>/dev/null || true
echo
echo "scratch_tree:"
find "$repo/.bclconvert_scratch" -maxdepth 3 -mindepth 1 -print -exec ls -ld {} \; 2>/dev/null | head -n 160 || true
echo
echo "slurm:"
squeue -j 2658 -o "%.18i %.9P %.12j %.8u %.2t %.10M %.6D %R" || true
sacct -j 2658 --format=JobID,JobName%40,State,ExitCode,Elapsed,AllocCPUS,NodeList -P 2>/dev/null || true
