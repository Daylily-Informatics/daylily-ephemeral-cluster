...
set -euo pipefail
venv=/tmp/dyec-illuminate-spike-20260530-oldbitstring
rm -rf "$venv"
python3 -m venv "$venv"
. "$venv/bin/activate"
python -m pip install --upgrade pip >/tmp/dyec-illuminate-oldbitstring-pip-upgrade.log 2>&1 || true
python -m pip install 'bitstring<4' illuminate >/tmp/dyec-illuminate-oldbitstring-pip-install.log 2>&1
python - <<'PY2'
