...
| CODE-003 | Versioning | Move durable fixes into pushed/tagged DayOA and DAY-EC versions before catalog success is treated as durable. | IN_PROGRESS | config_or_startup_contract | Gate 2 | orchestrator | DayOA branch/tag pushed: `ab160ed`, tag `2.0.27`. DAY-EC catalog, payload environment, and current docs now pin DayOA `2.0.27`; DAY-EC commit/tag/push still pending. |  |  |
