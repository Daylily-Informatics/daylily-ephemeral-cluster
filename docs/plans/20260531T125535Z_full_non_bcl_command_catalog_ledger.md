...
| RUN-017 | Live dyec-515 | Run `ont_run_qc`. | BLOCKED | dependency_unavailable | Gate 3 | orchestrator | `status_summary.json`: session `d515_0531t1608_r3_017_ont_run_qc`, exit `1`; run failed before DayOA because `/fsx/run_dir_mounts/20260513_ONT_HG003/` is missing. `docs/plans/20260531T170000Z_dyec515_runqc_runtime_retest/ont_mount_create.stderr.txt` shows `FSx file system fs-0192642b439759d7d already has 8 active DRAs.` Raw FSx inventory in `fsx_all_dras_after_ont_create_block.stdout.json` confirms 8 active DRAs. | The ONT run directory is not mounted and FSx has no available active-DRA slot. Freeing or rotating a DRA is a destructive/mutating operational decision requiring explicit approval. | Blocked pending an available DRA slot or explicit approval to remove/rotate an existing association. |
| RUN-018 | Live dyec-515 | Run `ultima_run_qc`. | BLOCKED | dependency_unavailable | Gate 3 | orchestrator | `status_summary.json`: session `d515_0531t1608_r3_018_ultima_run_qc`, exit `1`; run failed before DayOA because `/fsx/run_dir_mounts/602221-20260417_2346/` is missing. Raw FSx inventory in `fsx_all_dras_after_ont_create_block.stdout.json` confirms the same 8 active-DRA ceiling that blocked ONT mount creation. | The Ultima run directory is not mounted and FSx has no available active-DRA slot. Freeing or rotating a DRA is a destructive/mutating operational decision requiring explicit approval. | Blocked pending an available DRA slot or explicit approval to remove/rotate an existing association. |
| MON-001 | Monitoring | Keep launch batches bounded, record terminal status/log evidence, and do not leave local launch processes running. | SUCCESS | legitimate_safety_handling | Gate 4 | orchestrator | Launch/status/log artifacts are under `docs/plans/20260531T160830Z_dyec515_non_bcl_511_retest/` and `docs/plans/20260531T170000Z_dyec515_runqc_runtime_retest/`; all launched local command processes completed. |  | Evidence captured; no local launch process remains running. |
...
- `ont_run_qc` and `ultima_run_qc` remain blocked by missing run-directory mounts. The non-destructive ONT mount attempt failed before creation because `fs-0192642b439759d7d` already has 8 active DRAs; raw FSx inventory confirms 8 active associations: `/references/`, the ILMN run mount, and six staging DRAs.

## Final Status

Ledger terminal state: 25 `SUCCESS`, 2 `BLOCKED`, 0 working rows.

Objective state: partially complete. All non-BCL sample-analysis catalog commands passed on `dyec-515`, `illumina_run_qc` passed with the catalog-rendered command path, and BCL Convert commands were skipped by request. `ont_run_qc` and `ultima_run_qc` could not be completed without an available FSx DRA slot or explicit approval to remove/rotate an existing association.
