#!/usr/bin/env python3
"""Durable DYEC5127 DayOA command-catalog validation driver.
...
REGION = "us-west-2"
CLUSTER = "dyec5127"
EXECUTING_ENTITY = "dyec5127"
REPOSITORY = "daylily-omics-analysis"
EXPORT_ROOT = "s3://lsmc-ssf-sequencing-data/derived/validation/dyec5127/dyec5127"
...
def analysis_id_for(command_id: str, *, dry_run: bool) -> str:
    ...
    return f"ccv5127_{STAMP}_{ordinal:02d}_{safe_command_id(command_id)}{suffix}"
...
"""
    try:
        result = run_shell(
            ...
            timeout=300,
            poll_interval=5,
            comment="DYEC5127 command catalog validation Gate 0 preflight",
        )
...
def write_report() -> None:
    ...
    lines = [
        "# DYEC5127 Command Catalog Validation Report",
        "",
        f"Updated: {utc_now()}",
        "",
        "## Gate 0",
...
def run_plan(args: argparse.Namespace) -> int:
    ...
    for batch in SAMPLE_BATCHES:
        if len(batch) > args.max_active:
            raise RuntimeError(f"Internal batch exceeds max active: {batch}")
        run_batch(command_map, batch, args)

    ensure_run_mount("illumina")
    run_single(command_map, "illumina_run_qc", args)
    run_bcl_command(command_map, "illumina_bclconvert", args)
    run_bcl_command(command_map, "illumina_run_qc_bclconvert", args)

    ensure_run_mount("ont")
    run_single(command_map, "ont_run_qc", args)

    ensure_run_mount("ultima")
    run_single(command_map, "ultima_run_qc", args)
