# DYEC5127 Command Catalog Validation Ledger

Opened: 2026-06-03T09:06:02Z

## Objective

Run every `daylily-omics-analysis` command catalog command on `dyec5127`, capped at four active live workflows at a time, using existing control-data manifests/configs and grouping run-directory work by DRA mount.
...
- Execution entity: `dyec5127`
- Export prefix pattern: `s3://lsmc-ssf-sequencing-data/derived/validation/dyec5127/dyec5127/<analysis_id>/`
...
```bash
source ./activate
python docs/plans/20260603T090602Z_dyec5127_command_catalog_validation/driver.py --preflight-only
python docs/plans/20260603T090602Z_dyec5127_command_catalog_validation/driver.py
```

The driver:

- Stops at Gate 0 if `dyec5127` is not visible.
- Generates sample configs using `dyec samples stage --config-only`.
- Uses each catalog command's `dy_command`, `dryrun_dy_command`, `jobs`, `genome`, and `git_tag`.
- Launches via `dyec workflow launch`; it never invokes Snakemake directly.
...
- `dyec --json cluster describe --profile lsmc --region us-west-2 --cluster dyec5127` succeeds.
