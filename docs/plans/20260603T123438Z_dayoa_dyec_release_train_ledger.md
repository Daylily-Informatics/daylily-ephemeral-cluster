...
| REL-003 | DYEC | Commit dirty/untracked DYEC benchmark and ledger artifacts that belong to the current work. | SUCCESS | repo_state_publish | Gate 5 | Codex | Staged 386 untracked files under `bench_expts/` and `docs/plans/`, 3.7M total, with no untracked files outside those evidence roots; included in commit `e2e581462f971f2329d0da21624b77d640e343c3`. |  | Dirty benchmark and ledger artifacts were captured in the `5.1.28` release commit. |
| REL-004 | DYEC | Update DYEC DayOA pins to `2.0.41`, test, commit, push, tag `5.1.28`, and publish. | SUCCESS | dependency_pin_release | Gate 5 | Codex | Updated `pyproject.toml`, source and packaged command catalogs, current docs, and tests to DayOA `2.0.41`; `python -m pip install -e .` installed `daylily-omics-analysis==2.0.41`; `python -m pytest -q tests/test_repository_catalog.py tests/test_cli_registry_v2.py tests/test_packaged_defaults.py tests/test_resources_extraction.py tests/test_versioning.py tests/test_workflow.py` -> 207 passed; annotated tag `5.1.28` pushed and peels to `e2e581462f971f2329d0da21624b77d640e343c3`; clean tag worktree built `daylily_ephemeral_cluster-5.1.28-py3-none-any.whl` and `.tar.gz`; `twup` uploaded and probed; `pip index` and `pip download daylily-ephemeral-cluster==5.1.28` succeeded. |  | DYEC `5.1.28` is published with clean semver artifacts and DayOA `2.0.41` pins. |
...
- 2026-06-03T12:52Z: DYEC `5.1.28` annotated tag pushed and package published from a clean detached tag worktree; artifact names had no `dev` or local suffix.
- 2026-06-03T12:55Z: DYEC self pins updated to `5.1.28`; focused self-pin tests passed with 93 tests.
