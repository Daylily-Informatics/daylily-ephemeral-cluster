...
| HC-006 | DYEC | Commit, push, tag, build, publish, and verify next DYEC release. | SUCCESS | release_publish | Gate 5 | Commit `68f4dae7e7f2fe4e144d541a413728b1ab6c413c`; branch pushed to `origin/codex/dyec515-full-catalog-20260531`; annotated tag `5.1.30` pushed, tag object `13faf6b...`, peels to `68f4dae7...`; clean detached worktree `/Users/jmajor/.codex/worktrees/dyec-5130-publish` built `daylily_ephemeral_cluster-5.1.30-py3-none-any.whl` and `.tar.gz`; `twine check` passed; `twup` upload returned PyPI URL; `pip index`, no-cache `pip download`, and PyPI release JSON all confirmed `5.1.30`, wheel, and sdist. |  | DYEC `5.1.30` is published with exact release artifacts. |
| HC-007 | S3 | Publish updated headnode config assets to LSMC us-west-2 and Daylily public us-west-2 reference buckets with readback hash verification. | SUCCESS | release_publish | Gate 5 | Local and readback SHA-256 all `b202b87b98a4810964dfb39b748bb80c8d5eb49b491e7cbf52f8f9623968434c`; updated `s3://lsmc-dayoa-references-usw2/runtime_assets/cluster_boot_config/post_install_ubuntu_combined.sh`, `s3://daylily-omics-analysis-references-public/cluster_boot_config/post_install_ubuntu_combined.sh`, and `s3://daylily-references-public/cluster_boot_config/post_install_ubuntu_combined.sh`; backups written under each bucket's `cluster_boot_config/backups/` or `runtime_assets/cluster_boot_config/backups/` prefix with suffix `pre-headnode-cache-20260603T130619Z`. |  | The live reference-bucket bootstrap scripts match the released local script. |
| HC-008 | Cross-repo | Verify final DayOA/DYEC release train state. | SUCCESS | final_acceptance | Gate 5 | DayOA `git describe` -> `2.0.41`, `2.0.41...HEAD` -> `0 0`, PyPI JSON includes DayOA wheel and sdist; DYEC `git ls-remote` confirms tag `5.1.30`, PyPI JSON includes DYEC wheel and sdist; S3 heads show updated script size `18866`, content type `text/x-shellscript`, and 2026-06-03T13:06Z timestamps. |  | Code/config release, package-index publication, and S3 bootstrap publication are complete. |
...
- 2026-06-03T13:04Z: DYEC `5.1.30` wheel and sdist are visible on PyPI and downloadable.
- 2026-06-03T13:06Z: Updated post-install bootstrap script was copied to the LSMC and Daylily public reference buckets with backup objects and matching readback hashes.
- 2026-06-03T13:08Z: Final DayOA, DYEC tag/package, and S3 head-object verification completed; all rows terminal.
