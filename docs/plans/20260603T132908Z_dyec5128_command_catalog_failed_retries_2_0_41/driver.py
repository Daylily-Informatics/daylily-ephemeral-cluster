#!/usr/bin/env python3
"""Retry failed DYEC5128 command-catalog commands with DayOA 2.0.41.

This driver is intentionally narrow: it retries only commands that failed in
the initial command-catalog validation run, using the current catalog tag
2.0.41 and dy-r-only workflow launches.
"""
...
def analysis_id_for(command_id: str, *, dry_run: bool) -> str:
    ...
    return f"ccv5128_retry241_{STAMP}_{ordinal:02d}_{safe_command_id(command_id)}{suffix}"
...
def dy_r_command(command: Any, raw_dy_command: str) -> str:
    ...
    return f"source dyoainit; dy-a slurm {command.genome}; dy-r {dy_r_args}"
...
def run_plan(args: argparse.Namespace) -> int:
    ...
    failures = [
        ...
        if (last_event(row, "live") or {}).get("status") in {"FAILED", "BLOCKED"}
    ]
