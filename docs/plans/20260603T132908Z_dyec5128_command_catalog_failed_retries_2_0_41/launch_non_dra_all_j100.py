#!/usr/bin/env python3
"""Launch all non-DRA DYEC5128 command-catalog commands with DayOA 2.0.41 and -j 100.

This launcher excludes run-directory commands that require new DRA mounts.
It uses dy-r only and submits live workflows without waiting for terminal state.
"""
...
REGION = "us-west-2"
CLUSTER = "dyec5128"
EXECUTING_ENTITY = "dyec5128"
REPOSITORY = "daylily-omics-analysis"
EXPORT_ROOT = "s3://lsmc-ssf-sequencing-data/derived/validation/dyec5128/dyec5128"
...
def last_event(command_id: str, phase: str | None = None, status: str | None = None) -> dict[str, Any] | None:
...
def analysis_id_for(command_id: str, *, dry_run: bool) -> str:
    ...
    return f"ccv5128_nondra_j100_241_{STAMP}_{ordinal:02d}_{safe_command_id(command_id)}{suffix}"
...
def verify_reference_only_config(samples_path: Path, units_path: Path) -> None:
    problems: list[str] = []
            ...
            for row_number, row in enumerate(reader, start=1):
                for cell in row:
                    for match in re.findall(r"/fsx/[^\t,; ]+", cell):
                        ...
                        elif not cleaned.startswith("/fsx/references"):
                            problems.append(f"{path}:{row_number}: non-reference path {cleaned}")
    if problems:
        detail = "\n".join(problems[:40])
...
def ensure_sample_config(alias: str) -> tuple[Path, Path]:
    ...
    if prior:
        ...
        if samples_path.is_file() and units_path.is_file():
            verify_reference_only_config(samples_path, units_path)
...
"""
    try:
        result = run_shell(
            ...
            timeout=300,
            poll_interval=5,
            comment="DYEC5128 command catalog validation Gate 0 preflight",
        )
...
def force_j100(rendered: str) -> str:
    if "dy-r" not in rendered:
        raise RuntimeError(f"Rendered command lacks dy-r: {rendered}")
    if re.search(r"(?<!\S)-j\s+\d+", rendered):
        return re.sub(r"(?<!\S)-j\s+\d+", "-j 100", rendered)
    return f"{rendered} -j 100"


def dy_r_command(command: Any, raw_dy_command: str) -> str:
    """Render a DayOA command through dy-r only, forcing -j 100."""
    ...
    return force_j100(f"source dyoainit; dy-a slurm {command.genome}; dy-r {dy_r_args}")
...
def launch_workflow(command: Any, *, dry_run: bool, samples_file: Path | None, units_file: Path | None, run_context_file: Path | None) -> str:
    ...
    success = last_event(command_id, phase, "SUCCESS")
    if success:
        return str(success["session_name"])
    running = last_event(command_id, phase, "LIVE_RUNNING" if not dry_run else "DRYRUN_RUNNING")
    if running:
...
def command_can_live(command_id: str) -> bool:
    dry = last_event(command_id, "dryrun", "SUCCESS")
    if not dry:
        append_event(
            {
                "command_id": command_id,
...
def write_report() -> None:
    ...
    for command_id in COMMAND_ORDER:
        dry = last_event(command_id, "dryrun")
        live = last_event(command_id, "live")
        status = "PENDING"
...
def load_command_map() -> dict[str, Any]:
    ...
    wrong_tag = [
        f"{command_id}={command_map[command_id].git_tag}"
        for command_id in COMMAND_ORDER
        if str(command_map[command_id].git_tag) != "2.0.41"
    ]
    if wrong_tag:
        raise RuntimeError("Retry requires catalog git_tag 2.0.41; got " + ", ".join(wrong_tag))
    return command_map
...
def run_plan(args: argparse.Namespace) -> int:
    ...
    append_event(
        {
            "command_id": "_run_directory_dra_blocked",
            "phase": "defer",
            "status": "BLOCKED",
            "commands": DRA_BLOCKED_COMMANDS,
            "reason": "run-directory commands require DRA mounts; user requested launching non-run-directory commands first",
        }
    )
            ...
            append_event(
                {
                    ...
                    "phase": "dryrun",
                    "status": "BLOCKED",
                    "reason": f"input preparation failed: {exc}",
                }
            )
            continue
        session = launch_workflow(
    ...
    append_event(
        {
            "command_id": "_driver",
            "phase": "live_submit_all",
            "status": "SUBMITTED",
            "count": len(live_launched),
            "sessions": live_launched,
        }
    )
    write_report()
    return 0
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--max-active", type=int, default=99)
