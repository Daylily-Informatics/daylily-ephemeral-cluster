#!/usr/bin/env python3
"""Build NA00232/NA09677 hybrid HIOMR config using ILMN FASTQs plus ONT chip1+chip2 FASTQs."""
...
OUT_DIR = LOG_DIR / "hybrid_hiomr_na2_chip12_fastq_config"
STAMP = "20260606T120700Z"
...
def csv_paths(value: str) -> list[str]:
    paths = [part for part in value.split(",") if part]
    return sorted(paths)


def require_run_mount_paths(paths: list[str], label: str) -> None:
    bad = [path for path in paths if not path.startswith("/fsx/run_dir_mounts/")]
    if bad:
        raise SystemExit(f"{label} has non-run-mount paths: {bad[:3]}")
...
def main() -> int:
    ...
    for sample in NA_ORDER:
        ...
        for chip in CHIPS:
            ...
            require_run_mount_paths(paths, f"{sample} {chip} ONT_R1_PATH")
            ont_paths.extend(paths)
        ont_paths = sorted(ont_paths)
        require_run_mount_paths([ilmn["ILMN_R1_PATH"], ilmn["ILMN_R2_PATH"]], f"{sample} ILMN")

        out = {field: "" for field in unit_fields}
        out.update(ilmn)
        out["RUNID"] = "HYB-4Coriells-chip1-chip2"
        out["SAMPLEID"] = sample
    ...
    samples_path = OUT_DIR / f"{STAMP}_hybrid_hiomr_na2_chip12_samples.tsv"
    units_path = OUT_DIR / f"{STAMP}_hybrid_hiomr_na2_chip12_units.tsv"
    manifest_path = OUT_DIR / f"{STAMP}_hybrid_hiomr_na2_chip12_source_manifest.tsv"
