...
job_filter = __JOB_FILTER_JSON__
...
payload["active_jobs"] = jobs
payload["active_job_count"] = len(jobs)
...
def main() -> int:
    ...
    parser.add_argument("--job-filter", default="")
    args = parser.parse_args()

    remote_code = (
        REMOTE_TEMPLATE.replace("__SESSION_JSON__", json.dumps(args.session))
        .replace("__JOB_FILTER_JSON__", json.dumps(args.job_filter))
    )
