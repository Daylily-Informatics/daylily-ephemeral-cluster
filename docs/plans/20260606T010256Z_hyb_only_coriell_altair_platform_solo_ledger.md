...
| SRC-005 | Mounts | Verify or create read-only mount for ILMN BCLConvert FASTQ source. | IN_PROGRESS | feature_implementation | Gate 1 | Mount/Data Agent | `src005_mount_ilmn_fastq.stderr.txt`: waiter timed out after the approved long wait; `mounts_status_20260606T0252Z.json`: `ilmn-lh01121-b23ww2nlt4-fastq` lifecycle still CREATING. | Large DRA metadata association not yet AVAILABLE. | Continue read-only lifecycle polling; no retry, duplicate mount, or delete. |
| ONT-001 | ONT | Build `/fsx/scratch/ONT/NA00232_SMN_R1_all.fastq.gz` from barcode18 chip1, chip2, chip4. | IN_PROGRESS | feature_implementation | Gate 2 | ONT Agent | `headnode_stage_ont_data.sh`, `launch_ont_stage.stdout.txt`, `ont_stage_status_20260606T0237Z.stdout.txt`: concatenation running in tmux. |  | First sample temp file active under `/fsx/scratch/ONT/tmp/`. |
...
| ILMN-001 | ILMN | Stage Coriell ILMN FASTQs for NA00232, NA09677, NA03986, and NA05164 to `/fsx/scratch/ILMN/`. | OPEN | feature_implementation | Gate 2 | ILMN Agent |  |  |  |
| ILMN-002 | ILMN | Stage HG001-HG007 Altair `{a,b,c}` R1/R2 FASTQs from the same ILMN run only to `/fsx/scratch/ILMN/`. | OPEN | feature_implementation | Gate 2 | ILMN Agent |  |  |  |
