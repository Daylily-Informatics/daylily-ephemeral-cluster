...
echo "SUSPENDING ${ACTIVE[*]:-none} $(date -u +%Y-%m-%dT%H:%M:%SZ)"
for job_id in "${ACTIVE[@]}"; do
  echo "scontrol suspend ${job_id}"
  scontrol suspend "$job_id" || echo "SUSPEND_FAILED job=${job_id} rc=$?"
done
...
for job_id in "${ACTIVE[@]}"; do
  ...
  scontrol resume "$job_id" || echo "RESUME_FAILED job=${job_id} rc=$?"
