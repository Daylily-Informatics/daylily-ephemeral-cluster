...
| REL-004 | PASS | DYEC `10.0.4` committed, annotated-tagged, pushed, and installed locally; `dyec --json version` reports `10.0.4`. |
| FOCUS-001 | PASS | Focused `simple-test` dry-run with DYEC `10.0.4` rendered as `source dyoainit; dy-a local hg38; dy-r help -j 150 -p -k -T 0 -n --default-resources time=100`; workflow exit code `0`; evidence exported to `s3://lsmc-ssf-sequencing-data/derived/validation/dyec8c/command_catalog_results/10.0.0-20260608T055032Z/ubuntu/ccv_dryrun_simple-test_20260608T055032Z/`. |
...
| REPORT-001 | PASS | Current result reported: 14/14 non-run-directory catalog commands render and launch, but full catalog dry-run is not passing on `dyec8c` because the headnode role lacks required read-only EC2 instance-type discovery permission; no cluster create/update/delete commands were run. |
