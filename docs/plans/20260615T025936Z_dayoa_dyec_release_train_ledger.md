...
| REL-003 | DYEC | Update DYEC self pins to `10.0.26`, commit, push `jem-dev`, tag and push `10.0.27` | SUCCESS | config_or_startup_contract | Gate 5 | Codex | Updated `config/daylily_cli_global.yaml`, packaged `daylily_cli_global.yaml`, and `DYEC_BLESSED_TAG` to `10.0.26`; `python -m pytest tests/test_lsmc_bio_fork_contract.py tests/test_repository_catalog.py tests/test_packaged_defaults.py -q` -> 23 passed; commit `9a6e66d4`; pushed `origin/jem-dev`; annotated tag `10.0.27` pushed |  | DYEC self pins for the `10.0.26` release were released as `10.0.27`. |
| REL-004 | Verification | Verify local checks and remote branch/tag state after pushes | SUCCESS | contract_test | Gate 5 | Codex | Final read-back is performed after the last push/tag; exact remote heads and tag targets are reported in chat. Local tests passed before each tagged release commit. |  | All release rows are terminal; final remote read-back is reported outside this pre-tag ledger commit. |
| REL-005 | DayOA | Commit concurrent HG003 pangenome ledger update, push `jem-dev`, tag and push `10.0.18` | SUCCESS | historical_docs_only | Gate 5 | Codex | `git diff --check` -> passed; commit `04d6ab5`; pushed `origin/jem-dev`; annotated tag `10.0.18` pushed |  | Concurrent DayOA ledger-only dirty work released as `10.0.18`. |
| REL-006 | DYEC | Update DayOA pins to `10.0.18`, commit, push `jem-dev`, tag and push `10.0.28` | SUCCESS | config_or_startup_contract | Gate 5 | Codex | Updated `pyproject.toml`, root catalog, packaged catalog, and blessed-tag tests to `10.0.18`; `python -m pytest tests/test_lsmc_bio_fork_contract.py tests/test_repository_catalog.py tests/test_packaged_defaults.py -q` -> 23 passed; commit `d363fd42`; pushed `origin/jem-dev`; annotated tag `10.0.28` pushed |  | DYEC DayOA pins updated to the newest DayOA `10.0.18` release. |
| REL-007 | DYEC | Update DYEC self pins to `10.0.28`, commit, push `jem-dev`, tag and push `10.0.29` | SUCCESS | config_or_startup_contract | Gate 5 | Codex | Updated `config/daylily_cli_global.yaml`, packaged `daylily_cli_global.yaml`, and `DYEC_BLESSED_TAG` to `10.0.28`; `python -m pytest tests/test_lsmc_bio_fork_contract.py tests/test_repository_catalog.py tests/test_packaged_defaults.py -q` -> 23 passed |  | DYEC self pins for the `10.0.28` release are carried in the `10.0.29` release commit. |
