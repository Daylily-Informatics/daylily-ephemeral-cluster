...
| REL-006 | DYEC | Update DayOA pins to `10.0.18`, commit, push `jem-dev`, tag and push `10.0.28` | IN_PROGRESS | config_or_startup_contract | Gate 5 | Codex | Updated `pyproject.toml`, root catalog, packaged catalog, and blessed-tag tests to `10.0.18`; `python -m pytest tests/test_lsmc_bio_fork_contract.py tests/test_repository_catalog.py tests/test_packaged_defaults.py -q` -> 23 passed |  |  |
| REL-007 | DYEC | Update DYEC self pins to `10.0.28`, commit, push `jem-dev`, tag and push `10.0.29` | OPEN | config_or_startup_contract | Gate 5 | Codex | Pending |  |  |
