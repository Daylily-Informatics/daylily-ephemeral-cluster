...
| REL-005 | DYEC self pin | Update DYEC self pins after first DYEC release | SUCCESS | release_train | Gate 3 | `config/daylily_cli_global.yaml`; `daylily_ec/resources/payload/config/daylily_cli_global.yaml`; `tests/test_lsmc_bio_fork_contract.py`; `pytest tests/test_lsmc_bio_fork_contract.py tests/test_repository_catalog.py -q` -> 15 passed | DYEC self pins now point at `10.0.37`; final release tag planned as `10.0.38`. |
| REL-006 | Final verification | Verify tests, clean git state, annotated tags, pushed branch/tags, and manifest summary | IN_PROGRESS | acceptance | Gate 5 | Pending final commit/tag/push verification | Pending. |
