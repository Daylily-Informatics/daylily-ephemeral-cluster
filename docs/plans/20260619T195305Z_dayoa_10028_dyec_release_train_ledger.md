...
| DYEC-001 | DYEC DayOA pin | Update DYEC DayOA pin surfaces from `10.0.27` to `10.0.28`. | SUCCESS | `pyproject.toml`, both command catalog copies, and fork/catalog tests updated. `conda run -n DAY-EC python -m pytest tests/test_lsmc_bio_fork_contract.py tests/test_repository_catalog.py -q` -> `15 passed`. Commit `3a65931e`; annotated tag `10.0.48`; branch/tag pushed. | Intermediate DYEC release completed. |
| DYEC-002 | DYEC self-pin | After `10.0.48` is pushed, update source and payload self-pins to `10.0.48`. | SUCCESS | `config/daylily_cli_global.yaml`, payload global config, and fork-contract test updated. `conda run -n DAY-EC python -m pytest tests/test_lsmc_bio_fork_contract.py tests/test_repository_catalog.py -q` -> `15 passed`. | Pending commit, tag `10.0.49`, and push. |
