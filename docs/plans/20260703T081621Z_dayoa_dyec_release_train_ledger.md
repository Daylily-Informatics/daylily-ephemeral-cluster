...
|---|---|---|---|---|---|---|---|---|---|
| ORCH-001 | Ledger | Record Gate 0, selected versions, dirty baseline, and terminal row counts. | IN_PROGRESS | feature_implementation | Gate 0 | orchestrator | This ledger; DayOA release evidence recorded; DYEC first-release validation recorded. |  | Final terminal counts pending both DYEC tags. |
...
| DYEC-001 | DYEC DayOA pin | Update DYEC DayOA pins to `10.0.53`, include dirty DYEC work, commit to `jem-dev`, push branch, tag `10.0.81`, push tag. | IN_PROGRESS | feature_implementation | Gate 2 | orchestrator | DYEC DayOA pins updated from `10.0.52` to `10.0.53`; `pytest tests/test_cli_registry_v2.py tests/test_repository_catalog.py tests/test_lsmc_bio_fork_contract.py tests/test_workflow.py tests/test_sbatch_wrapper.py tests/test_headnode_init.py tests/test_packaged_defaults.py tests/test_triplets.py tests/test_cluster_request_config.py tests/test_renderer.py tests/test_script_entrypoints.py -q -> 363 passed`; `python -m py_compile daylily_ec/cli.py daylily_ec/workflow/create_cluster.py daylily_ec/scripts/daylily_run_omics_analysis_headnode.py`; `git diff --check`. |  | Commit/tag pending. |
| DYEC-002 | DYEC self pin | Update DYEC self pins to `10.0.81`, commit to `jem-dev`, push branch, tag `10.0.82`, push tag. | OPEN | feature_implementation | Gate 2 | orchestrator | Pending self-pin update, tests, commit, branch push, and tag push. |  |  |
| VAL-001 | Validation | Run focused local checks sufficient for the release train and verify annotated tags. | IN_PROGRESS | contract_test | Gate 5 | orchestrator | DayOA `14 passed`; DYEC `363 passed`; compile and whitespace checks passed; DayOA tag type verified. |  | DYEC tag verification pending. |
