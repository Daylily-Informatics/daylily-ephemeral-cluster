...
| REL-003 | DYEC release 1 | Commit changed DYEC work, push `jem-dev`, create/push annotated tag `10.0.92`. | OPEN |  |  |
| REL-004 | DYEC self-pin | Update DYEC self-pins to `10.0.92` in active and packaged global config/tests. | OPEN |  |  |
| REL-005 | DYEC release 2 | Commit self-pin update, push `jem-dev`, create/push annotated tag `10.0.93`. | OPEN |  |  |
| REL-006 | Verification | Verify tests and final pushed tags. | IN_PROGRESS | `source ./activate && pytest -q && bash -n config/day_cluster/post_install_ubuntu_combined.sh && bash -n config/day_cluster/post_install_rhel8_dragen.sh` | Pre-`10.0.92` DYEC verification passed: 1162 passed, 8 skipped; boot scripts passed `bash -n`. |
