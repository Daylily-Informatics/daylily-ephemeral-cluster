...
| REL-004 | DYEC Self Pin | Update DYEC self pins to the final release version `10.0.105`. | IN_PROGRESS | config_or_startup_contract | Gate 5 | Codex | `config/daylily_cli_global.yaml`, packaged payload copy, and `tests/test_lsmc_bio_fork_contract.py` updated to `10.0.105`. |  |  |
