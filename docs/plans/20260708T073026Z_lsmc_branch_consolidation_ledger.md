...
| BC-002 | DYEC | Move current local dirty DYEC changes onto `jem-dev`, test, and push to `lsmc-bio`. | IN_PROGRESS | feature_implementation | Gate 1 | Codex | Switched from local `jemdev10` to local `jem-dev`, fast-forwarded to `origin/jem-dev`, reapplied dirty work; `pytest tests/test_cli_registry_v2.py tests/test_export.py -q -> 180 passed`. |  |  |
...
| BC-004 | DYEC/DayOA | Delete `lsmc-bio` remote `jemdev10` refs after safe consolidation. | OPEN | feature_implementation | Gate 1 | Codex | DYEC branch exists and is ancestor; DayOA branch absent. |  |  |
...
| BC-006 | DYEC | Preserve this ledger in the branch consolidation commit. | IN_PROGRESS | contract_test | Gate 5 | Codex | Ledger path: `docs/plans/20260708T073026Z_lsmc_branch_consolidation_ledger.md`. |  |  |
