...
| DRG-006 | Both | Final verification: clean local repos, annotated tags, pushed refs, and exact final version report. | SUCCESS | DayOA: `git status --short --branch -> ## jem-dev...origin/jem-dev`, `git describe -> 10.0.67-0-gc0acb42`, `git cat-file -t 10.0.67 -> tag`, origin tag visible, `git merge-base --is-ancestor origin/jem-dev-dragen origin/jem-dev -> 0`. DYEC: `git status --short --branch -> ## jem-dev...origin/jem-dev`, `git describe -> 10.0.112-1-gfc1aa197` before this final ledger commit, `git cat-file -t 10.0.111 -> tag`, `git cat-file -t 10.0.112 -> tag`, both origin tags visible, `git merge-base --is-ancestor origin/jem-dev-dragen origin/jem-dev -> 0`. Pin scan shows active DayOA `10.0.67` and DYEC self-pin `10.0.111`. | Final report tags: DayOA `10.0.67`; DYEC DayOA-pin release `10.0.111`; DYEC self-pin release `10.0.112`. |
