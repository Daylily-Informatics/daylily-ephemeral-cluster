...
| DYEC-RELEASE-1 | DYEC | Commit dirty DYEC work plus docs and DayOA pins, push `jem-dev`, create and push annotated `10.0.115` | SUCCESS | Commit `9270bf6a`; pushed `origin/jem-dev`; `git cat-file -t 10.0.115 -> tag`; pushed tag `10.0.115`; remote tag dereferences to `9270bf6a056118bf5ff89c1723f335cc99c4cc77`. |
| DYEC-SELF-PIN | DYEC | Update DYEC self-pin surfaces to `10.0.115` | SUCCESS | Updated `config/daylily_cli_global.yaml`, packaged global config, `config/dragen_fix5_repo_overrides.txt`, and self-pin test constant to `10.0.115`; also updated the DRAGEN fix override DayOA ref to `10.0.69`. |
| DYEC-VERIFY-2 | DYEC | Run focused validation for self-pin release | SUCCESS | `pytest tests/test_lsmc_bio_fork_contract.py tests/test_packaged_defaults.py tests/test_workflow.py -q` -> 116 passed; `git diff --check` clean; remote tag `10.0.116` unused before final release. |
| DYEC-RELEASE-2 | DYEC | Commit self-pin, push `jem-dev`, create and push annotated `10.0.116` | OPEN | Pending final commit/tag after this ledger update. |
