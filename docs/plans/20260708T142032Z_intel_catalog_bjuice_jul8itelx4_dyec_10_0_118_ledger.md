...
| MOUNT-001 | Run mounts | Resolve or create Illumina run DRA for `illumina_run_qc` without blocking sample-analysis launch | IN_PROGRESS | feature_implementation | Gate 1 | Agent 13 | mount_id=`20260514_LH01106_0009_B23TVLGLT4`, association=`dra-0e31c89df77176d37`, lifecycle=`CREATING`, created_at=`2026-07-08T15:51:45Z` |  | Waiting for FSx DRA availability |
| MOUNT-002 | Run mounts | Resolve or create ONT run DRA for `ont_run_qc` without blocking sample-analysis launch | IN_PROGRESS | feature_implementation | Gate 1 | Agent 14 | mount_id=`20260513_ONT_HG003`, association=`dra-0a60db320f05c1eb1`, lifecycle=`CREATING`, created_at=`2026-07-08T15:51:44Z` |  | Waiting for FSx DRA availability |
| MOUNT-003 | Run mounts | Resolve or create Ultima run DRA for `ultima_run_qc` without blocking sample-analysis launch | IN_PROGRESS | feature_implementation | Gate 1 | Agent 15 | mount_id=`602221-20260417_2346`, association=`dra-03ebcc27fecb99c6f`, lifecycle=`CREATING`, created_at=`2026-07-08T15:51:44Z` |  | Waiting for FSx DRA availability |
...
| CMD-013 | Run-QC command | Launch, monitor, export, verify, and cleanup `illumina_run_qc` | OPEN | feature_implementation | Gate 2 | Agent 13 | analysis_id=`ccv_live_illumina_run_qc_20260708T142032Z` |  |  |
| CMD-014 | Run-QC command | Launch, monitor, export, verify, and cleanup `ont_run_qc` | OPEN | feature_implementation | Gate 2 | Agent 14 | analysis_id=`ccv_live_ont_run_qc_20260708T142032Z` |  |  |
| CMD-015 | Run-QC command | Launch, monitor, export, verify, and cleanup `ultima_run_qc` | OPEN | feature_implementation | Gate 2 | Agent 15 | analysis_id=`ccv_live_ultima_run_qc_20260708T142032Z` |  |  |
