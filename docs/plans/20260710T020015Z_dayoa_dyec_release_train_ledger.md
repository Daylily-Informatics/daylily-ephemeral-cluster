...
| REL-004 | DYEC | Verify and commit every current dirty/untracked change, push `jem-dev`, create and push annotated `10.0.130` | IN_PROGRESS | feature_implementation | Gate 5 | Combined focused suite -> `244 passed`; source/payload parity and evidence secret scan passed; catalog driver tmux is idle; staged non-generated-TSV `git diff --check` passed | Generated TSV rows retain terminal tab delimiters for empty final fields; all other staged content is whitespace-clean. |
| REL-005 | DYEC | Self-pin active and packaged DYEC config to `10.0.130` and update contract pins | OPEN | config_or_startup_contract | Gate 2 | Four-file self-pin inventory from prior release contract | |
| REL-006 | DYEC | Verify self-pin, commit, push `jem-dev`, create and push annotated `10.0.131` | OPEN | feature_implementation | Gate 5 | Tag confirmed unused locally after remote tag fetch | |
