...
| DYEC-002 | DYEC | Propagate the endpoint during headnode/compute configuration and reject stale file-only generated config | SUCCESS | config_or_startup_contract | Gate 2 | 8 | Ubuntu/RHEL source and packaged post-install paths plus readiness tests |  | Supported live configure and fresh-node inheritance both passed after publication. |
...
| DOC-001 | DYEC | Publish an operator runbook for server lifecycle, client migration, rotation, HA expansion, and rollback | SUCCESS | feature_implementation | Gate 5 | 9 | `docs/sentieon_license_server_runbook.md`, SHA-256 `6d541a01b28297a1e42e4c697ecee666125c32a3ea4d271d25c40483ce8ed299` |  | Includes expiry gates, identity-policy-only secret access, explicit service restart approval, pinned CloudWatch installation, and immutable bootstrap rules. |
