...
| AZ-008 | Release boundary | Commit and push only owned, validated changes to `sentieon-single`; do not create or move a release tag without an explicit version request | OPEN | legitimate_safety_handling | Gate 5 | orchestrator | Validation and owned-file staging pending |  |  |
