...
| REL-001 | DYEC release | Focused/full validation, commit, push branch, and cut next unused annotated pin release | RUNNING | contract_test | Gate 3 | orchestrator | Full suite: `1540 passed, 11 skipped`; focused Ruff clean; source/payload comparisons, exact fifth-config YAML assertions, and `git diff --check` passed |  | Commit, push, and annotated tag pending. |
| REL-002 | DYEC self-pin | Advance source and packaged DYEC self-pin to REL-001, validate, commit, push, and cut next patch tag | OPEN | contract_test | Gate 3 | orchestrator | Pending REL-001 |  |  |
