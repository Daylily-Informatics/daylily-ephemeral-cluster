...
| DYREL-006 | DYEC accounting-fix self-pin | Advance source/payload DYEC self-pin to DYREL-005 and publish the following annotated patch tag | IN_PROGRESS | contract_test | Gate 5 | orchestrator | Source/payload global config and fork contract now pin `10.3.12`; focused suite `269 passed`; source/payload comparison, Ruff, and diff checks clean |  | Commit, push, and annotated tag pending. |
