...
| DYREL-005 | DYEC accounting dry-run fix | Publish the live-proven exact-message handling for ParallelCluster update dry runs | IN_PROGRESS | contract_test | Gate 5 | orchestrator | Live accounting attach on `ifx-p2-1000-120-0715` completed through the supported DYEC path; focused runner/attach suite `39 passed`; full suite `1542 passed, 11 skipped`; Ruff, source/payload comparison, and diff checks clean | The installed LSMC ParallelCluster fork returns rc 1 with the canonical successful dry-run message. | Commit, push, and annotated tag pending. |
| DYREL-006 | DYEC accounting-fix self-pin | Advance source/payload DYEC self-pin to DYREL-005 and publish the following annotated patch tag | OPEN | contract_test | Gate 5 | orchestrator | Pending DYREL-005 |  |  |
| ENV-001 | Local DYEC environment | From the DYEC checkout, run `source ./activate` and install the released checkout editable | SUCCESS | contract_test | Gate 3 | orchestrator | `pip install -e .` completed; installed distribution metadata is `daylily-ephemeral-cluster 10.3.11`; imported `daylily_ec` resolves to this exact worktree; `git describe` is `10.3.11` and the tree is clean before this ledger update |  | The CLI has no `--version` option, so verification used Python distribution metadata and exact import path. |
