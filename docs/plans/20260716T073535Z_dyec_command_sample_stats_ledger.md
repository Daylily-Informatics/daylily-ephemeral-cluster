...
| R2 | Feature release | Commit and push the combined candidate on `codex/dyec-command-sample-stats`, then create and verify annotated tag `10.3.22` without merging to `main`. | IN_PROGRESS | feature_implementation | Gate 5 | Codex | User explicitly requested a feature-branch release. Remote tag availability will be rechecked immediately before creation. |
| R3 | Self-pin release | Advance both source/package DYEC defaults to `10.3.23`, commit, push, and create a second verified annotated tag. | OPEN | config_or_startup_contract | Gate 5 | Codex | This is the requested double-DYEC release; final reported DYEC version will be the exactly self-pinned `10.3.23`. |
