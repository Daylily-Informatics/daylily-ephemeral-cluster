...
| DYEC-002 | DYEC | Test, commit, push, and publish first DYEC release | IN_PROGRESS | contract_test | Gate 3 | Focused pin/catalog gate -> `74 passed`; Ruff clean | First DYEC release carries the DayOA pin. |
| DYEC-003 | DYEC | Advance both DYEC self-pin copies to the first DYEC release | OPEN | config_or_startup_contract | Gate 4 | Pending | No self-pin may remain one version behind. |
| DYEC-004 | DYEC | Test, commit, push, and publish follow-up DYEC release | OPEN | contract_test | Gate 5 | Pending | Final release is the self-pinned DYEC version. |
