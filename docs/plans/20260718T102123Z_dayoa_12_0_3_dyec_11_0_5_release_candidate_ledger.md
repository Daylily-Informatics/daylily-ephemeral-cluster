...
| DYEC-004 | Functional commit | Commit catalog, tests, and release-candidate evidence without changing the DYEC self-pin. | IN_PROGRESS | Focused gates are green; commit pending. | This is the first commit in the requested train. |
| DYEC-005 | Self-pin | Advance both shipped DYEC bootstrap refs and exact tests to `11.0.5` in a distinct commit. | OPEN | Pending the functional commit. | No tag is created in this preparation lane. |
| DYEC-006 | Validation | Pass focused release gates, source/payload parity, YAML parsing, Ruff, and `git diff --check`. | OPEN | Pending. | Full-suite execution will be attempted if the activated environment is available. |
| DYEC-007 | Handoff | Return a clean local branch with exact commits and no publication or live mutations. | OPEN | Pending. | Parent integrator owns PR, merge, and annotated tag. |
