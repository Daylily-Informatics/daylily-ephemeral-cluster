...
| DYEC-006 | Release | Merge reviewed patch to the `13.x` release branch and publish immutable annotated tag `13.0.5` | OPEN | feature_implementation | Gate 5 | orchestrator | Git commit, PR, checks, merge, tag evidence |  |  |
| URSA-001 | Ursa | Pin Ursa exactly to released DAY-EC `13.0.5`, test, release, and deploy only Ursa services | BLOCKED | config_or_startup_contract | External ledger | orchestrator | Ursa controlling ledger `docs/plans/20260719T122247Z_ursa_10_0_1_owy_seqrunqc_acceptance_ledger.md` | DAY-EC `13.0.5` is not yet released. | Unblocks after `DYEC-006` succeeds. |
