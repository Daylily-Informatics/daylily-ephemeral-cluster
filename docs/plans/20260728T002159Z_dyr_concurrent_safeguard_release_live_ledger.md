...
| REL-02 | R2 | Pin both active DayOA catalog copies and exact test contracts to `13.0.61`; publish annotated `15.0.13`. | IN_PROGRESS | Only semantic active pin fields may change; historical validation facts remain historical. |
| REL-03 | R3 | Advance only DYEC bootstrap/self-pin copies to `15.0.13`; publish annotated `15.0.14`. | OPEN | Begins only after REL-02 tag is remotely verified. |
