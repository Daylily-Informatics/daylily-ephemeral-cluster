...
| DYEC-004 | Ursa integration | Fold `ursa-changes` into the feature branch and resolve any catalog/test overlap without dropping either change. | OPEN | feature_implementation | Gate 1 | Merge only after the DayOA pin and catalog work is committed. |
| DYEC-005 | First release | Validate, commit/push the integrated branch, create/verify/push annotated `16.1.8`. | OPEN | release | Gate 6 | Must include the explicit DayOA `13.0.90` pin. |
