#!/usr/bin/env python3
"""Collect a compact read-only Take333lc report evidence bundle from the headnode."""
...
def main() -> int:
    ...
    archive_path = f"/tmp/codex_take333lc_report_evidence_{uuid.uuid4().hex}.tar.gz"
    script = r"""set -euo pipefail
export DAYOA_AGENT_ID=codex-take333lc-report-20260804
export DAYOA_AGENT_KIND=codex
export DAYOA_HUMAN_REQUESTOR='John Major'
export DAYOA_TMUX_SESSION=dayoa_take333lc_hg002_5x5x_20260804
export DAYOA_LEDGER_PATH='/Users/jmajor/projects/lsmc/daylily-ephemeral-cluster/docs/plans/20260804T173058Z_take222_take333_final_report_ledger.md'
dyec analysis visit --analysis-root __ROOT__ --mode read --intent 'Collect compact Take333lc report evidence'
cd __ROOT__/daylily-omics-analysis
evidence_dir=$(mktemp -d /tmp/take333lc-report-evidence.XXXXXX)
...
raw_keys = [
    ...
    'multiqc_sentdhiomr2_truvari',
    'multiqc_sentdhiomr2_jasmine_le50_rtg',
...
(out / 'take333lc_multiqc_extract.json').write_text(
...
with (out / 'take333lc_output_inventory.tsv').open('w', encoding='utf-8', newline='') as handle:
...
(out / 'take333lc_output_inventory_summary.json').write_text(
    json.dumps(summary, indent=2, sort_keys=True), encoding='utf-8'
)

manifest_payload = {}
for path in sorted(results.glob('deliveries/inflection_hiomr2_analytical_callsets_vcf_gvcf/take333lc/*/package_manifest.json')):
    manifest_payload[path.as_posix()] = json.loads(path.read_text(encoding='utf-8'))
(out / 'take333lc_inflection_package_manifests.json').write_text(
...
for name in (
    ...
    'hiomr2_take333lc_hg002_5x5x.yaml',
    'take333lc_manifest_validation_receipt.json',
    'take333lc_bjuice_preval_config_receipt.json',
):
...
other_reports = out / 'other_reports'
other_reports.mkdir()
for path in sorted((results / 'other_reports').glob('*.tsv')):
    shutil.copy2(path, other_reports / path.name)
for path in (
    results / 'reports/benchmarks_summary.tsv',
    results / 'reports/benchmarks.tsv',
    results / 'reports/benchmark_rejections.tsv',
    results / 'reports/dayoa_evidence_manifest.json',
):
    if path.exists():
        destination = out / 'reports' / path.name
        destination.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(path, destination)
PY
...
""".replace("__ROOT__", ROOT).replace("__ARCHIVE__", archive_path)
    try:
        result = run_shell(
            ...
            as_user="ubuntu",
            timeout=600,
            comment="Collect Take333lc report evidence",
        )
    ...
    if not size_match or not sha_match:
        raise RuntimeError("Take333lc archive size or SHA-256 was not returned")
...
"""
        try:
            chunk_result = run_shell(
                ...
                as_user="ubuntu",
                timeout=120,
                comment=f"Read Take333lc evidence chunk {chunk_index + 1}",
            )
        except SsmCommandFailedError as exc:
            print(exc.result.stdout)
            if exc.result.stderr:
    ...
    run_shell(
        ...
        as_user="ubuntu",
        timeout=120,
        comment="Remove Take333lc temporary evidence archive",
    )
