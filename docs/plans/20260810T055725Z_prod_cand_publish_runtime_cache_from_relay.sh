...
relay_prefix="derived/validation/prod-cand-260809-init-test-x2-20260810/cache-promotion/20260810T055725Z/cached_envs"
dest_bucket="lsmc-dayoa-references-usw2"
dest_prefix="runtime_assets/cached_envs"
state_dir="/Users/jmajor/projects/lsmc/daylily-ephemeral-cluster/docs/plans/20260810T055725Z_prod_cand_runtime_cache_publish_state"
...
finish() {
  ...
  printf '%s\n' "${rc}" > "${rc_file}"
  printf 'CANONICAL_PUBLISH_%s\t%s\trc=%s\n' "${state}" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "${rc}"
}
...
printf 'DESTINATION_PREFLIGHT_COMPLETE\tconda=%s\tcontainers=0\n' "${#names[@]}"

tmp_root="$(mktemp -d)"
trap 'rm -rf -- "${tmp_root}"; finish' EXIT

for name in "${names[@]}"; do
  relay_yaml_key="${relay_prefix}/conda/${name}.yaml"
  wait_deadline=$((SECONDS + 21600))
  while true; do
    set +e
    relay_head_output="$(aws s3api head-object \
      --bucket "${relay_bucket}" \
      --key "${relay_yaml_key}" 2>&1)"
    relay_head_rc=$?
    set -e
  ...
  relay_count="$(aws s3api list-objects-v2 \
    --bucket "${relay_bucket}" \
    --prefix "${relay_prefix}/conda/${name}/" \
    --max-keys 1 \
    --query KeyCount \
    --output text)"
  [[ "${relay_count}" != "0" ]] || {
    printf 'ERROR_RELAY_PREFIX_EMPTY\t%s\n' "${name}" >&2
    exit 26
  }

  relay_uri="s3://${relay_bucket}/${relay_prefix}/conda/${name}/"
  dest_uri="s3://${dest_bucket}/${dest_prefix}/conda/${name}/"
  printf 'PUBLISH_START\t%s\t%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "${name}"
  ...
  aws s3 cp --only-show-errors \
    "s3://${relay_bucket}/${relay_prefix}/conda/${name}.yaml" \
    "${local_yaml}"
  ...
  printf 'PUBLISH_COMPLETE\t%s\t%s\n' "${completed}" "${name}"
done

printf 'CANONICAL_PUBLISH_VERIFIED\t%s\tconda=%s\tcontainers=0\n' \
