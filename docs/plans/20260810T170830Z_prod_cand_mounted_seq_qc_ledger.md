...
| QC-012 | Release activation | Publish a DYEC release containing QC-011 and activate the exact released CLI. | IN_PROGRESS | feature_implementation | Gate 1 | Codex | Initial DYEC `16.1.67` release exposed a source-version resolver defect: an unrelated dirty worktree rendered `16.1.68.dev0` despite `HEAD` being exactly tagged. The next patch honors exact semver tags before `setuptools-scm`; the focused DYEC suite is 112 passed. | `setuptools-scm` guess-next-dev treats a dirty exact-tag checkout as a post-tag development version. | Release `16.1.68`, activation proof, and fresh workflows remain pending. |
