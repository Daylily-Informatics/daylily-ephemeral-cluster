...
| QC-022 | ONT final fresh rerun | Launch a new ONT-only analysis from the released scoped catalog and verify the complete final FSx report tree. | SUCCESS | feature_implementation | Gate 1 | Codex | `prodcand_ont_seqqc_13422_r3` at exact DayOA `13.4.22` completed its ten-job ONT QC DAG with workflow exit `0`; FSx contains the native ONT report tree and MultiQC HTML. DYEC-only post-controller export created DRA `dra-0126a4d61163319f8`; task `task-0f786441dbda1fc49` succeeded, the association detached/deleted with `delete_data_in_file_system: false`, and both ONT MultiQC HTML files are present in the exact S3 validation prefix. | The live ParallelCluster configuration omitted its configured export bucket from HeadNode `S3Access`, despite the current DYEC template declaring it with write access. A temporary least-privilege headnode role policy enabled the required DYEC export visit and DRA validation records. | Exported to `s3://lsmc-dayoa-analysis-results-usw2/validation/prod-cand-260809/ubuntu/prod-cand-ont-seqqc-13422-r3/`; the FSx root remains present. |
