...
| QC-015 | DYEC ONT repair release | Pin source and packaged catalog defaults to DayOA `13.4.18`, advance the DYEC self pin, then publish and activate the release. | IN_PROGRESS | feature_implementation | Gate 1 | Codex | Source/package catalog and self-pin edits are complete; DYEC focused suite: 144 passed and source/payload config files are byte-identical. |  | Publish annotated `16.1.69` and prove the activated CLI identity before retry. |
| QC-016 | ONT fresh live verification | Launch one fresh ONT catalog analysis at the repaired DayOA/DYEC tags and verify its final FSx report tree. | IN_PROGRESS | feature_implementation | Gate 1 | Codex | User authorized continued debugging until final FSx reports succeed; old ONT root is retained as failure evidence and will not be reused. |  | Wait for QC-015, then launch a fresh ONT-only ID and restore the authorized terminal monitor. |
