...
| REL-001 | Release | Commit, push, PR/merge, annotate/push the retarget-plan DYEC patch release. | SUCCESS | active_product_contract | Gate 5 | orchestrator | PR [#106](https://github.com/lsmc-bio/daylily-ephemeral-cluster/pull/106), merged `5a7d75d7`; annotated tag/release `18.0.2`. | Exact checkout at `18.0.2` used for the first dry controller. |
| DYEC-003 | DYEC | Permit only catalog-materialized runtime manifests, controller log, and DayOA provenance ledgers while retaining pinned source immutability. | IN_PROGRESS | production_bugfix | Gate 5 | orchestrator | First dry session `prod-cand-1703-hg002-bjuice-v2-retargeted-multiau-dry-20260814T225600Z` stopped before `dy-r`; session `...dry2-20260814T230419Z` ran `dy-r -n` successfully but verifier returned `25` afterward. | The list stays closed: nine catalog artifacts plus exact root `analysis_artifacts.tsv` and `artifact_lineage.tsv`; no directory-level exemption. |
| REL-002 | Release | Commit, push, PR/merge, annotate/push the DYEC patch that contains the completed DYEC-003 list. | OPEN | active_product_contract | Gate 5 | orchestrator | Pending PR/release evidence. | Required before a third, fresh dry analysis root is launched. |
| RUN-001 | Operations | Generate the seven manifests, dry-run exactly seven AU roots with zero Slurm jobs, and launch a fresh controller with the release pins. | ATTEMPTING_BUGFIX | active_product_contract | Gate 5 | orchestrator | `...dry2-20260814T230419Z` reported `dy-r` return `0`, zero jobs; controller terminal `25` solely on the post-run immutable guard. | Use a distinct fresh root after DYEC-003 is released. |
