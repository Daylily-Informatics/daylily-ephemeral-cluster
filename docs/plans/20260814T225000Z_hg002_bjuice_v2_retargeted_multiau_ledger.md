...
| DYEC-003 | DYEC | Permit only catalog-materialized runtime manifests and controller log while retaining pinned DayOA source immutability. | IN_PROGRESS | production_bugfix | Gate 5 | orchestrator | Dry session `prod-cand-1703-hg002-bjuice-v2-retargeted-multiau-dry-20260814T225600Z` terminated `rc=25`, zero Slurm jobs. | The former verifier rejected its own nine exact runtime artifacts. The repair is a closed path allowlist, not a directory-level exemption. |
| REL-002 | Release | Commit, push, PR/merge, annotate/push the DYEC patch that contains DYEC-003. | OPEN | active_product_contract | Gate 5 | orchestrator | Pending PR/release evidence. | Required before a new dry analysis root is launched. |
| RUN-001 | Operations | Generate the seven manifests, dry-run exactly seven AU roots with zero Slurm jobs, and launch a fresh controller with the release pins. | ATTEMPTING_BUGFIX | active_product_contract | Gate 5 | orchestrator | DYEC catalog dry session above; zero jobs submitted. | First dry stopped at the immutable-checkout guard before `dy-r`; use a distinct fresh root after DYEC-003 is released. |
