...
| DOC-001 | Advance active operator documentation and release-contract expectations to DYEC `18.0.18` / DayOA `15.0.10`. | IN_PROGRESS | contract_test | Gate 2 | Active operator docs and the source-version expectation are updated. | The exact version assertion requires the immutable `18.0.18` tag and will be rerun after tag creation. |
...
| REL-001 | Commit, merge, annotated-tag, and publish GitHub release `18.0.18`. | OPEN | feature_implementation | Gate 5 | New non-`v` patch release follows `18.0.17`. | Pending validation and release. |
