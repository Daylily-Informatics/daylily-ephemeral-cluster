...
| STV2-006 | Documentation | Publish schema/location/CLI semantics and export-evidence expectations. | SUCCESS | feature_implementation | 5 | `docs/execution_status_v2.md` (DayOA) and `docs/clone_resident_execution_status_v2.md` (DYEC) document the schema/location/no-legacy contract; analysis export now requires post-SUCCEEDED clone-status evidence. |
| STV2-007 | Releases | Cut DayOA `15.0.19`, pin all current catalog commands, then cut DYEC `18.0.29` with annotated tags. | SUCCESS | feature_implementation | 5 | DayOA tag object `77176068eddfe85724fc562832a128008ee71579` peels to `7c3377fd543f6c987736f81792159452646510d4`; DYEC tag object `8ffe85a5cdb33720920ab236e287f035a065d601` peels to `908863fe545a61c797127906ca0be9b38d5e6de6`. Both are annotated and pushed. Source/payload catalog parity holds; all 30 top-level and 30 `dyec_builds.current` tags are `15.0.19`. |
...
| STV2-009 | Compatibility removal | Remove the opt-in/export compatibility path so all analysis exports require strict clone-resident v2 evidence; release DYEC `18.0.30`. | SUCCESS | feature_implementation | 5 | Generic analysis exports now preflight the full root, require v2 evidence, and have no compatibility flag. Runtime-cache export uses its distinct explicit export kind. Focused DYEC suite: 433 passed. Annotated remote tag object `81f0940afd3f08cc3c04ec9f5d851b49b383fd84` peels to `bad195d6417197817a6443cdb289699825d6575d`. |
...
| AMD-003 | Remove analysis-export compatibility rather than preserving it as an opt-in flag. | User direction: “i do not want any backward compatability for this maintained, i want it removed.” No historic data is deleted; unsupported old/missing v2 receipts fail closed. | SUCCESS |

## Completion criteria

The source objective requires STV2-001 through STV2-007 and STV2-009 to be `SUCCESS`. The operational objective remains incomplete until STV2-008 receives explicit authorization and is independently proven.
