...
| STV2-009 | Compatibility removal | Remove the opt-in/export compatibility path so all analysis exports require strict clone-resident v2 evidence; release DYEC `18.0.30`. | IN_PROGRESS | feature_implementation | 5 | User explicitly rejected backward compatibility. Generic analysis exports now preflight the full root and require v2 evidence; runtime-cache export uses its distinct explicit export kind. Focused DYEC suite: 433 passed; release commit/tag/push pending. |
...
| AMD-003 | Remove analysis-export compatibility rather than preserving it as an opt-in flag. | User direction: “i do not want any backward compatability for this maintained, i want it removed.” No historic data is deleted; unsupported old/missing v2 receipts fail closed. | IN_PROGRESS |
