...
| RUN-04 | HIOMR2 | `hiomr2_slim_kitchensink_mega`, 5x ILMN + 5x ONT slim-data. | IN_PROGRESS | active_product_contract | Gate 1 | Dry attempt `ed173b24…bb4e` reached `rc=0` / zero submitted work; same-root live attempt `bb42a3c8…e08` is running (`98/293`, Slurm `358` RUNNING) with no failure marker. |
...
| RUN-09 | SOLO CG | `complete_genomics_cg_snv_concordance`, CG slim-data. | IN_PROGRESS | active_product_contract | Gate 1 | Dry attempt `5ef4ed24…d55` reached `rc=0` / zero submitted work; same-root live attempt `d5d0cb54…3543` is running (`10/80`, one Slurm job RUNNING) with no failure marker. |
