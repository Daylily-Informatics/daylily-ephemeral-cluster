...
| HN-05 | Headnode | Force-configure `pclu-18045` with exact released DYEC `18.0.54`; prove remote version before the fresh 15.0.34 Ultima pangenome dry root. | SUCCESS | legitimate_safety_handling | Gate 0 | A fresh authoritative inventory reported zero live controllers and zero Slurm jobs. The exact-release `--force` configure then completed; remote `dyec version` reports `18.0.54` and `dyec analysis --help` succeeds. |
...
| CAT-04 | Catalog | Render the fresh replacement Ultima pangenome command from `18.0.54`; prove DayOA `15.0.34`, exact same flags, a new root, and declared explicit input manifest. | SUCCESS | active_product_contract | Gate 0 | Local exact-release render for `pclu18045_rc0_18054_pangenomeultima_20260819T040635Z` resolved only DayOA `15.0.34`, explicit Ultima six-manifest inputs, graph target `produce_sentpg_ug_snv_vcf`, final MultiQC/evidence targets, `-j 333 -T 0 -p`, and dry-only `-n`. Launch remains correctly gated by HN-05. |
...
| RUN-10 | Pangenome ILMN | `illumina_sentieon_pangenome_kitchensink`, ILMN slim-data. | SUCCESS | active_product_contract | Gate 1 | The `15.0.29` dry attempt `4cba625c…0fa` failed before submitted work when the current sentd selector requested a graph/doppelmark path. The replacement `15.0.30` dry attempt `b0c441ec…0775` reached `rc=0` / zero submitted work; its same-root live attempt `55afaa62…99d3` reached controller/day-run/Snakemake `rc=0`. |
| RUN-11 | Pangenome Ultima | `ultima_sentieon_pangenome_kitchensink`, Ultima slim-data. | IN_PROGRESS | active_product_contract | Gate 1 | The `15.0.29` dry attempt `6a08b710…4c` failed before submitted work because `pre_prep_ultima_cram` ambiguously claimed the canonicalized graph CRAM. The `15.0.30` dry attempt `a4e7ee2f…e3` then reached `rc=0` / zero submitted work, but its same-root live attempt `4c44b356…9294` ended controller/day-run/Snakemake `rc=1` when `samtools reheader` received SAM. DayOA `15.0.34` corrects that exact pipe. Under fresh root `pclu18045_rc0_18054_pangenomeultima_20260819T040635Z`, dry attempt `0e1ecb2e…b1db` reached controller/day-run/Snakemake `rc=0` with zero submitted jobs; same-root live attempt `44dfd8c8…f2f` is active with only `-n` removed. |
