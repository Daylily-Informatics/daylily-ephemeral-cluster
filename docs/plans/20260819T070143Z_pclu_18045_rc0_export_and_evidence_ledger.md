...
`runtime_cache_export` used the supported `dyec runtime-cache export` path and never used raw S3 copy/sync/move.  Its first staging attempt failed semantic copy verification; the corrected second attempt timed out with `rc=137` before temporary DRA creation, S3 delivery, or a completion marker.  The terminal receipts are summarized in `..._artifacts/runtime_cache_export_failures.md`.  No retry, cleanup, cache-import release, or region-specific reference-bucket publication is authorized from this ledger state.
