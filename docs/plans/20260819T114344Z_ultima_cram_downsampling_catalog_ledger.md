...
| DYE-007 | Evidence truth | Record new runs as dry-only; do not relabel prior live evidence or claim runtime/read-count proof | SUCCESS | legitimate_safety_handling | Gate 5 | root | Current and `18.0.59` append one scoped validation record per command with `dryrun_status=success`, `live_status=not_run`, actual dirty-candidate executable base `20325e84`, candidate commit `8fd55dbd`, DayOA `15.0.38`/`95cabdbf`, local terminal receipts, and explicit no-read-count/no-performance limitations; prior live records and their S3 prefixes remain unchanged |  | Dry evidence truth is explicit and historical evidence is preserved |
| DYE-008 | Release | Commit, push feature branch, create/push annotated `18.0.59`, and verify tag type/commit | SUCCESS | feature_implementation | Gate 5 | root | Test-only commit `20325e84`; catalog/dry-artifact candidate commit `8fd55dbd`; clean final evidence commit on `codex/downsample_ultima_cram_in_config`; branch and annotated `18.0.59` tag pushed without force after remote-absence recheck; tag type and peeled commit verified in the release handoff |  | Published without moving or overwriting a ref |
| DYE-009 | Final report | Terminalize every row and record changed files, validation, non-actions, and residual risk | SUCCESS | plan_amendment | Gate 5 | root | Both repositories have terminal ledgers; exact commits/tags, local suites, two dry receipts, zero-submission evidence, and explicit exclusions are recorded here and in the companion DayOA ledger |  | Objective complete at the requested dry-only release gate |
...
Status counts: SUCCESS 10; other terminal states 0.

## Release Gate Evidence

- DayOA dependency: annotated tag `15.0.38`, release commit `95cabdbfd35e57bd8974ec0b6583f3f2dd4b3202`.
- Catalog shape: `current == 18.0.59`; allowed DayOA tags are `15.0.37` and `15.0.38`; exactly the two requested Ultima kitchen-sink commands pin `15.0.38`; the other 29 remain at `15.0.37`; `default_ref` remains `15.0.37`.
- Historical integrity: every pre-existing numeric snapshot, including `18.0.58`, equals its tagged source; canonical and packaged catalogs are byte-identical.
- Dry-only proof: solo analysis `pclu18045_u075_solo_18059_15038_20260819t130742z` and pangenome analysis `pclu18045_u075_pang_18059_15038_20260819t130742z` each exited controller/day-run/Snakemake rc=0 and submitted zero jobs. Each rendered exactly one threaded `pre_prep_ultima_cram` command using seed `33.75`; the pangenome canonicalizer consumes that prepared CRAM.
- Post-dry boundary: authoritative controller count returned to zero. The sole Slurm job remained the unrelated pre-existing job `1604`; it was not touched.
- Excluded by design: no live workflow, CRAM read-count validation, performance benchmark, export, Slurm intervention, or infrastructure mutation.
