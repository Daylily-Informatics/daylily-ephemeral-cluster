...
Controlling request: implement the DYEC lane of the approved multi-agent plan by adding public, JSON-capable `dyec cluster compute-fleet` and `dyec slurm-accounting recover` contracts. DYEC owns every underlying ParallelCluster call. This lane performs no live AWS mutation, production recovery, build, deployment, release tag, or package publication.
...
- Both commands are registered public DYEC commands with `supports_json`, `mutates_state`, and `long_running` policy.
...
- Recovery requires the exact persisted cluster configuration path, a trimmed explicit AWS profile, resolved AWS account identity, exact stack/database/user, and paired missing-accounting creation/cost acknowledgement flags. Exact mode never auto-selects another stack or discovers/reconciles a PrivateLink fallback.
...
| DYEC-009 | Git handoff | Commit and push the isolated `codex/` branch | OPEN | feature_implementation | Gate 5 | DYEC agent | Pending commit/push evidence |  |  |
