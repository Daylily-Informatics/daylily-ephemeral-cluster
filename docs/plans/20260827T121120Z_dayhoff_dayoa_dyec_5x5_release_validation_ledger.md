...
| DAY-001 | DayOA | Restore supported generic-scope core argv, prove isolated routed CNV/SV behavior, implement product-scoped manifests, obtain exact dry `rc=0`, and publish annotated tag `16.0.12`. | ATTEMPTING_BUGFIX | active_product_contract | Gate 3 | Scoped manifests are pushed at candidate checkpoint `f8dfa6b6` on draft PR #128. Five fresh zero-submission capsules are terminal. The newest, `...t1835z`, proved that the HG002 Truvari aggregate still requested Jasmine-derived query roots while Jasmine was explicitly disabled. DayOA now filters all five Jasmine-derived Truvari projections from the aggregate when disabled while retaining independent CNVscope, LongReadSV, Sniffles2, TIDDIT, and hard-SNV projections; focused Jasmine/Truvari/scoped-manifest tests pass (`31 passed`). | Push the corrected candidate and create a new fresh exact-candidate capsule proving normal core/CNV/SV/receipt/report/package/manifest closure with no Ganon2 or Jasmine job. |
...
| RUN-002 | Dry proof | Launch one fresh exact-candidate dry controller and obtain attributable `rc=0` with zero submitted workflow jobs. | ATTEMPTING_BUGFIX | contract_test | Gate 6 | Candidate DayOA explicitly disables Ganon2. The fourth capsule `...t1657z` is terminal after the obsolete global `--summary` artifact inventory expanded disabled Jasmine. The fifth capsule, `codex-hiomr2-16012-hg002-5x5-20260827t1835z`, used DayOA `f8dfa6b6` and terminated controller/DayOA `rc=2` before formal Snakemake execution because the enabled HG002 Truvari aggregate hard-coded disabled Jasmine-derived query roots. Both capsules submitted zero jobs. The rule-owner correction makes Jasmine config authoritative for those query roots; focused tests pass (`31 passed`). | Push the correction and create a new analysis identity/root. Prove zero Ganon2 and Jasmine jobs, explicit `bjuice_manifest` closure, retained DAG PNG generation, and preserve the resulting analysis ID/root/checkout/config as the later released live-capsule shape. |
