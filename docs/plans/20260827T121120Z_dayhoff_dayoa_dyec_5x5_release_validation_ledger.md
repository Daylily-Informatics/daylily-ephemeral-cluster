...
| AMD-MANIFEST-001 | Provenance/catalog | Remove the global pre-run artifact inventory, retain stable DAG PNG rendering, and add explicit scoped manifest targets for ILMN/ONT/ULTIMA SeqQC, Bjuice, and ILMN/ULTIMA/ONT/CG solo kitchensink. | SUCCESS | plan_amendment | Gate 0 | Explicit user approval; DayOA has eight rule-owned same-schema manifests whose rows are all `important` and whose written paths are declared inputs, including stable `dags/dag.png`. DYEC source and packaged catalogs are byte-identical, add the approved target to active and immutable `19.0.33` commands, remove the obsolete global producer switch, and default only DAG generation on. After two terminal capsules exposed the stale filename and exact allowlist gaps, capsule `codex-hiomr2-16012-hg002-5x5-20260827t191249z` returned controller/DayOA/Snakemake `rc=0`, submitted zero jobs, and captured identical nonempty 2,028,642-byte source/evidence DAG PNGs. | The global pre-run inventory is gone; stable DAG evidence and product-scoped manifests are proven through the ordinary catalog target. |
...
| DAY-001 | DayOA | Restore supported generic-scope core argv, prove isolated routed CNV/SV behavior, implement product-scoped manifests, obtain exact dry `rc=0`, and publish annotated tag `16.0.12`. | SUCCESS | active_product_contract | Gate 3 | Final exact-head capsule `codex-hiomr2-16012-hg002-5x5-20260827t192115z` returned controller/DayOA/Snakemake `rc=0` with zero submissions after the clear-text log repair; `470 passed, 1 skipped` across HIOMR2 tests and all three CodeQL checks passed. PR #128 merged normally as `186f673a`; annotated tag `16.0.12` peels to that merge and is published on origin. | DayOA `16.0.12` is released with the supported core and sex-routed isolated CNV/SV contracts. |
| DYEC-001 | DYEC | Set global and command-catalog DayOA pins exactly to released `16.0.12`, add explicit manifest targets to SeqQC, solo-kitchensink, and Bjuice+Inflection commands, validate, and publish annotated tag `19.0.33`. | IN_PROGRESS | config_or_startup_contract | Gate 4 | The isolated candidate sets `CURRENT_DYEC_BUILD=19.0.33`, retargets active/global commands to released DayOA `16.0.12`, adds the eight approved scoped-manifest targets to the corresponding command families, removes the global manifest producer option, defaults DAG rendering on, and keeps the two catalog copies byte-identical. Post-release focused pin/preflight/controller tests are `36 passed`; exact candidate dry proof is terminal. | Commit this terminal release checkpoint, verify a clean tree and absent remote tag, then publish annotated tag `19.0.33`. |
...
| RUN-001 | Inputs/catalog | Validate the exact 5x ILMN plus 5x ONT six-manifest fixture and render the complete ordinary Bjuice plus Inflection packaging command with DYEC `19.0.33`, DayOA `16.0.12`, and `bjuice_manifest`. | SUCCESS | contract_test | Gate 6 | The packaged `hg002_bjuice_verified_5x5x_fastq` six-manifest fixture renders the ordinary targets `produce_sentdhiomr2_slim_kitchensink_mega`, `produce_sentdhiomr2_inflection_analytical_package`, and `bjuice_manifest`; the command uses the normal six-manifest staging contract, hg38, and the 5x ILMN/5x ONT fixture. | No recall target, alternate fixture, previous-artifact inputs, or reduced target substitution is present. |
| RUN-002 | Dry proof | Launch one fresh exact-candidate dry controller and obtain attributable `rc=0` with zero submitted workflow jobs. | SUCCESS | contract_test | Gate 6 | Final exact-source capsule `codex-hiomr2-16012-hg002-5x5-20260827t192115z` used DayOA `f500050c` and DYEC `5c92a254`; controller, DayOA, and Snakemake each returned `rc=0` and zero jobs were submitted. Its ordinary 118-job closure contains no Ganon2 or Jasmine job. | The exact candidate dry gate is terminal; failed predecessor capsules remain terminal and were not reused. |
