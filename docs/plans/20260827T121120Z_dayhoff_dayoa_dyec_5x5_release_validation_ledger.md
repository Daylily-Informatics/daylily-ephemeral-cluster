...
| DH-002 | Dayhoff | Run focused and broad attributable tests, merge through a green PR without admin override, and push annotated tag `9.0.30`. | IN_PROGRESS | contract_test | Gate 2 | PR [#197](https://github.com/lsmc-bio/dayhoff/pull/197) is at `c5714b9`. Review hardening fixed deployment-scoped paths, direct-sidecar credential isolation, strict cleanup S3 destination shape, retired-contract tests, real refresh identity validation, and the Ursa `11.0.50` pin. Focused tests pass `288/288`; pin/manifest tests pass `29/29`; Ruff and diff checks pass. The 12 remaining non-E2E failures reproduce exactly on pristine `9.0.29`; E2E is unconfigured without `KAHLO_BASE_URL`. | Normal review/check clearance, merge, and annotated tag remain. |
