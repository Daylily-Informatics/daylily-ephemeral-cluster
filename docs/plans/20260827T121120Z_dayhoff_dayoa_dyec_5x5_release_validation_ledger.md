...
| DAY-001 | DayOA | Restore supported generic-scope core argv, prove isolated routed CNV/SV behavior, implement product-scoped manifests, obtain exact dry `rc=0`, and publish annotated tag `16.0.12`. | IN_PROGRESS | active_product_contract | Gate 3 | Candidate `e2516ff5` is pushed and draft PR #128 is open; prior focused `659 passed, 1 skipped` and exact baseline comparison remain valid. Four fresh zero-submission capsules are terminal; the newest exposed the global planner-time artifact inventory described by `AMD-MANIFEST-001`. | Complete scoped manifests, then create a new fresh exact-candidate capsule and prove normal core/CNV/SV/receipt/report/package/manifest closure with no Ganon2 job. |
...
| RUN-002 | Dry proof | Launch one fresh exact-candidate dry controller and obtain attributable `rc=0` with zero submitted workflow jobs. | IN_PROGRESS | contract_test | Gate 6 | Candidate DayOA explicitly disables Ganon2. The fourth capsule `...t1657z` is terminal after the obsolete global `--summary` artifact inventory expanded disabled Jasmine; it submitted zero jobs. A new analysis identity/root is required after `AMD-MANIFEST-001`. | Prove zero Ganon2 jobs, explicit `bjuice_manifest` closure, retained DAG PNG generation, and preserve the resulting analysis ID/root/checkout/config as the later released live-capsule shape. |
