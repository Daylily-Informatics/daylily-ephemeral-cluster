...
| DAY-001 | DayOA | Restore supported generic-scope core argv, prove isolated routed CNV/SV behavior, implement product-scoped manifests, obtain exact dry `rc=0`, and publish annotated tag `16.0.12`. | IN_PROGRESS | active_product_contract | Gate 3 | Capsule `codex-hiomr2-16012-hg002-5x5-20260827t191249z` used clean DayOA checkpoint `41fbc943` with DYEC `ae31f055`; controller, DayOA, and Snakemake returned `rc=0`, zero jobs were submitted, and the expected 118-job closure contained no Ganon2 or Jasmine job. Broad HIOMR2 contracts pass (`482 passed, 1 skipped`), and the release-ledger checkpoint is published at `f6e49d86`. | Merge the green PR normally and publish annotated tag `16.0.12`. |
...
| RUN-002 | Dry proof | Launch one fresh exact-candidate dry controller and obtain attributable `rc=0` with zero submitted workflow jobs. | SUCCESS | contract_test | Gate 6 | Final capsule `codex-hiomr2-16012-hg002-5x5-20260827t191249z` used DayOA `41fbc943` and DYEC `ae31f055`; controller, DayOA, and Snakemake each returned `rc=0`, zero jobs were submitted, and the identical source/evidence stable DAG PNGs are nonempty at 2,028,642 bytes. The expected 118-job closure contains no Ganon2 or Jasmine job. | The exact candidate dry gate is terminal; failed predecessor capsules remain terminal and were not reused. |
