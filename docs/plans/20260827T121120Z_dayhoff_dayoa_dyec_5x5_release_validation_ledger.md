...
| RUN-002 | Dry proof | Launch one fresh exact-candidate dry controller and obtain attributable `rc=0` with zero submitted workflow jobs. | ATTEMPTING_BUGFIX | contract_test | Gate 6 | Capsule `codex-hiomr2-16012-hg002-5x5-20260827t1900z` used DayOA `4f9f6621`; DayOA/Snakemake returned `rc=0`, zero jobs were submitted, and the expected 118-job closure included core, finalize/gather, GVCFtyper, routed CNV/SV, five independent Truvari queries, receipts, reports, both packages, and `bjuice_manifest`, with no Ganon2 or Jasmine job. The outer controller alone returned `rc=24` because its evidence monitor looked for timestamped DAG filenames. The DYEC monitor now consumes the stable `dags/dag.png` contract. | Create a new analysis identity/root using DayOA `41fbc943` and the corrected DYEC checkpoint. Require controller/DayOA/Snakemake `rc=0`, zero submissions, and an attributed nonempty stable DAG PNG. |
