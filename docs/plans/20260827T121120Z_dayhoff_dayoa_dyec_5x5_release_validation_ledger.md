...
| DAY-001 | DayOA | Restore supported generic-scope core argv, prove isolated routed CNV/SV behavior, obtain exact dry `rc=0`, and publish annotated tag `16.0.12`. | IN_PROGRESS | active_product_contract | Gate 3 | Candidate `e2516ff5` is pushed and draft PR #128 is open; prior focused `659 passed, 1 skipped` and exact baseline comparison remain valid, and the explicit Ganon-disable amendment passes 101 focused tests. Three earlier fresh zero-submission capsules are terminal; Ganon2 is now intentionally outside the release DAG. | Create a new fresh exact-candidate capsule and prove normal core/CNV/SV/receipt/report/package closure with no Ganon2 job. Full fresh male HG002 operation remains the release proof. |
...
| RUN-002 | Dry proof | Launch one fresh exact-candidate dry controller and obtain attributable `rc=0` with zero submitted workflow jobs. | IN_PROGRESS | contract_test | Gate 6 | Candidate DayOA `e2516ff5` explicitly disables Ganon2, so the unfinished external database is no longer a dry-DAG dependency. A new analysis identity/root is required; the three failed capsules stay terminal. | Prove zero Ganon2 jobs and preserve the resulting analysis ID/root/checkout/config as the later released live-capsule shape. |
