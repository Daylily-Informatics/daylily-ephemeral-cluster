...
| AMD-MANIFEST-001 | Provenance/catalog | Remove the global pre-run artifact inventory, retain stable DAG PNG rendering, and add explicit scoped manifest targets for ILMN/ONT/ULTIMA SeqQC, Bjuice, and ILMN/ULTIMA/ONT/CG solo kitchensink. | IN_PROGRESS | plan_amendment | Gate 0 | Explicit user approval; DayOA now has eight rule-owned same-schema manifests whose rows are all `important` and whose written paths are declared inputs, including stable `dags/dag.png`. DYEC source and packaged catalogs are byte-identical, add the approved target to active and immutable `19.0.33` commands, remove the obsolete global producer switch, and default only DAG generation on. Focused DYEC contract tests: `13 passed, 23 skipped`; expanded changed-surface suite: `314 passed, 23 skipped, 18 failed`, with the exact 18 pre-existing candidate mismatches and no manifest/preflight failure. Independent shell-token inspection confirms each approved target occurs exactly once in active and `19.0.33` commands. Catalog YAML parsing and `git diff --check` pass. The failed `...t1657z` capsule terminated `rc=2` with zero submissions during the removed global `--summary`; its Python manifest builder never ran. | Obtain the exact fresh production-shaped `dy-r -n` proof for parser, DAG, and catalog closure. |
...
| DAY-001 | DayOA | Restore supported generic-scope core argv, prove isolated routed CNV/SV behavior, implement product-scoped manifests, obtain exact dry `rc=0`, and publish annotated tag `16.0.12`. | ATTEMPTING_BUGFIX | active_product_contract | Gate 3 | Six fresh zero-submission capsules are terminal. The newest, `...t1845z`, used DayOA `af0eb79d` and proved that enabled HG002 Truvari/CNV copy-state diagnostics still requested legacy `hybrid-cli170` CNV VCF/TBI paths as unconditional Snakemake inputs instead of consuming the isolated CNV component receipt. DayOA checkpoint `acbd2446` now makes all independent Truvari projections and CNV copy-state receipt-backed, resolves only receipt-authorized PASS artifacts, terminalizes upstream failures diagnostically, and passes the explicit active query set to the aggregate. Broad HIOMR2 contracts pass (`481 passed, 1 skipped`). | Create a new fresh exact-candidate capsule proving normal core/CNV/SV/receipt/report/package/manifest closure with no Ganon2 or Jasmine job. |
...
| RUN-002 | Dry proof | Launch one fresh exact-candidate dry controller and obtain attributable `rc=0` with zero submitted workflow jobs. | ATTEMPTING_BUGFIX | contract_test | Gate 6 | Candidate DayOA explicitly disables Ganon2 and Jasmine. Capsule `...t1835z` exposed disabled-Jasmine query expansion. Capsule `codex-hiomr2-16012-hg002-5x5-20260827t1845z` used DayOA `af0eb79d`, terminated controller/DayOA `rc=2` before formal Snakemake execution, and exposed unconditional legacy CNV VCF/TBI inputs in receipt-based diagnostics. Both submitted zero jobs. The receipt-aware correction is pushed at DayOA `acbd2446`; broad HIOMR2 contracts pass (`481 passed, 1 skipped`). | Create a new analysis identity/root. Prove zero Ganon2 and Jasmine jobs, receipt-only dependency closure for optional caller artifacts, explicit `bjuice_manifest` closure, retained DAG PNG generation, and preserve the resulting analysis ID/root/checkout/config as the later released live-capsule shape. |
