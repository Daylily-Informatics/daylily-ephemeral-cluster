...
| DAY-001 | DayOA | Restore supported generic-scope core argv, prove isolated routed CNV/SV behavior, implement product-scoped manifests, obtain exact dry `rc=0`, and publish annotated tag `16.0.12`. | ATTEMPTING_BUGFIX | active_product_contract | Gate 3 | Seven fresh zero-submission capsules are terminal. Capsule `...t1845z` exposed legacy VCF inputs in receipt-based diagnostics. Capsule `codex-hiomr2-16012-hg002-5x5-20260827t1853z` used receipt-aware DayOA `acbd2446`, reached formal Snakemake job rendering, then terminated controller/DayOA/Snakemake `rc=1` because five new Hybrid metadata dictionary literals were not escaped for Snakemake shell formatting. DayOA checkpoint `4f9f6621` doubles every affected brace and adds a source guard. Broad HIOMR2 contracts pass (`482 passed, 1 skipped`). | Create a new fresh exact-candidate capsule proving normal core/CNV/SV/receipt/report/package/manifest closure with no Ganon2 or Jasmine job. |
...
| RUN-002 | Dry proof | Launch one fresh exact-candidate dry controller and obtain attributable `rc=0` with zero submitted workflow jobs. | ATTEMPTING_BUGFIX | contract_test | Gate 6 | Candidate DayOA explicitly disables Ganon2 and Jasmine. Capsules `...t1835z` and `...t1845z` exposed disabled-Jasmine and receipt-dependency gaps. Capsule `...t1853z` used DayOA `acbd2446`, reached formal Snakemake rendering, and terminated attributable controller/DayOA/Snakemake `rc=1` with zero submissions because new Hybrid metadata dictionaries had unescaped Snakemake braces. The correction is pushed at DayOA `4f9f6621`; broad HIOMR2 contracts pass (`482 passed, 1 skipped`). | Create a new analysis identity/root. Prove zero Ganon2 and Jasmine jobs, receipt-only dependency closure for optional caller artifacts, explicit `bjuice_manifest` closure, retained DAG PNG generation, and preserve the resulting analysis ID/root/checkout/config as the later released live-capsule shape. |
