...
CURRENT_DEFAULT_DAYOA_RELEASE = "16.0.1"
...
def test_18_0_46_remains_frozen_and_pinned_only_to_dayoa_15_0_27() -> None:
    ...
    assert raw["dyec_builds"][PREVIOUS_DYEC_RELEASE]["dayoa_git_tags"] == ["15.0.26"]
    assert raw["dyec_builds"][LEGACY_DYEC_RELEASE] == legacy["dyec_builds"][
        LEGACY_DYEC_RELEASE
    ]
    assert raw["dyec_builds"][LEGACY_DYEC_RELEASE]["dayoa_git_tags"] == ["15.0.24"]


def test_18_0_46_all_eleven_production_lanes_have_exact_rc0_controller_shapes() -> None:
    ...
    for command_id in ALL_LANE_IDS:
        ...
        argv = command.launch_argv(
            analysis_id="release-18046-catalog-render",
...
def test_18_0_46_pangenome_lanes_use_graph_targets_without_a_pangenome_genome_build() -> None:
    catalog = load_repository_catalog(SOURCE_CATALOG)
...
def test_18_0_46_leaves_legacy_dev_pangenome_closures_unmodified() -> None:
    raw = _raw_catalog()
    legacy = _release_tag_catalog(LEGACY_DYEC_RELEASE)
    commands = raw["dyec_builds"][DYEC_RELEASE]["commands"]
    baseline_commands = legacy["dyec_builds"][LEGACY_DYEC_RELEASE]["commands"]
