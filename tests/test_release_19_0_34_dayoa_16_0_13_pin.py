...
DYEC_RELEASE = "19.0.34"
PREVIOUS_DYEC_RELEASE = "19.0.33"
DAYOA_RELEASE = "16.0.13"
...
def test_19_0_34_is_current_and_uniformly_pins_released_dayoa() -> None:
...
def test_19_0_34_preserves_19_0_33_and_changes_only_its_dayoa_pin() -> None:
    raw = _catalog()
    tagged = _tag_catalog(PREVIOUS_DYEC_RELEASE)
    historical = tagged["dyec_builds"][PREVIOUS_DYEC_RELEASE]
    previous = raw["dyec_builds"][PREVIOUS_DYEC_RELEASE]
    release = raw["dyec_builds"][DYEC_RELEASE]

    assert previous == historical
    expected_commands = copy.deepcopy(historical["commands"])
    for command in expected_commands.values():
        command["git_tag"] = DAYOA_RELEASE

    assert release["commands"] == expected_commands
    assert release["aliases"] == historical["aliases"] == {}


def test_19_0_34_keeps_the_production_bjuice_inflection_command_shape() -> None:
    raw = _catalog()
    active = {
    ...
    assert active["git_tag"] == release["git_tag"] == DAYOA_RELEASE
