...
def test_current_snapshot_history_and_packaged_payload_retain_active_semantics() -> None:
    ...
    assert raw["repositories"]["daylily-omics-analysis"]["default_ref"] == "16.0.3"
    assert {
        command["git_tag"]
    ...
    assert raw["dyec_builds"]["current"] == raw["dyec_builds"]["19.0.3"]
    assert raw["dyec_builds"]["current"]["dayoa_git_tags"] == ["16.0.3"]
    assert {
        command["git_tag"]
