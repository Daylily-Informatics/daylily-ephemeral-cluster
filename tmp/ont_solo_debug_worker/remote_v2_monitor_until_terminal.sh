...
for i in $(seq 1 120); do
  ...
  shm=$(ssh -o BatchMode=yes -o ConnectTimeout=5 "${NODE}" "bash -l -c 'df -h /dev/shm | tail -1; find /dev/shm -maxdepth 1 -type d -name \"sentdont_tmp_*\" -printf \"%p \" 2>/dev/null || true'" 2>/dev/null || true)
...
echo "== scontrol 182 final =="
scontrol show job "${JOB}" 2>&1 || true
echo "== sacct 182 final =="
sacct -j "${JOB}" -o JobID,JobName%80,State,ExitCode,NodeList%30,AllocCPUS,ReqMem,Elapsed,Start,End -P 2>/dev/null || true
echo "== sentdont log tails =="
